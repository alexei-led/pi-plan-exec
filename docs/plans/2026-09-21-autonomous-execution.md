...
Status: round 12 confirmation passed on the corrected pins; all PRs remain draft
...
- Main through `3f1a329`: `npm run test:all` passed lint, TypeScript, all 537 tests (zero failures or skips) and package validation; `npm run test:runtime-smoke` passed the real controller, Bridge/native RPC, owned workers/checks, required review, promotion and archive with the corrected pins.
...
- Native `e78595340ff36ad481e205c0872fadba9412227d`: typecheck and the focused external-job follow-up suite (44 integration tests including bounded expiry, omitted/explicit unbounded lifetime and conflicting replay) pass. Full suite: 3337/3361 unit and 1101/1109 integration passed, 14/7 skipped; the 10 unit and 1 integration failure are environment-only Darwin failures (`/var` versus `/private/var` canonicalization and unix-socket `EINVAL`) reproduced unchanged on `2b2d2e3`.
...
- Fusion `b113212469bb5d2d0ad7bb5ff98e7cec12e25c7e`: 258 unit / 132 integration / one E2E pass, including three cross-process delayed panel recovery cases, revision-tip compaction and a compacted-revision recreate fence; typecheck, lint and package pass.
...
Round `08-final` reviewed through main `541aaf3`, native `689d937`, Bridge `3e99ec2`, Fusion `fc4c0ac` and Revmux `988904f`. It completed with two of two sources, no degradation and four confirmed major findings. Lost-reply optional review cancellation must recover a terminal identity before proving retirement; final verification cannot be waived into an unrecoverable archive state. Cross-process Fusion reconciliation must not overwrite a newer judge identity with a delayed panel snapshot. Native external-job follow-up must preserve the explicitly selected execution lifetime. The Bridge skip lookup also needs its v2 owner, found while sweeping the same recovery path. Upstream native changes were merged additively with cost RPC and Pi 0.87 support. Corrections are integrated on main `3f1a329`, native `e785953` and Fusion `108c52c`: required final verification can no longer be waived, skip recovery validates owner, digest and lifetime and retains a lost-reply cancelled identity, Fusion fences concurrent snapshots with a durable revision chain compacted to one tip, native follow-up propagates the requested lifetime, and registry locking uses a kernel `flock` instead of stale-lock eviction. The 519-test gate and successful CI on `541aaf3` certify the prior revision only.

Round `09-final` degraded to one of two sources (Codex usage limit) and found nothing; `10-final` replaced it with the `claude-only` roster over the same cumulative scope and returned four of four sources, no degradation, one code major and four minor findings. The major was unbounded Fusion revision growth with whole-chain re-reads on every refresh; it is corrected by tip compaction with post-write tip verification. The minors were stale pins/gate text in `docs/runtime-contracts.md`, the superseded lock description and missing module row in `docs/architecture.md`, the still-offered finalize waiver in `skills/exec-plan/SKILL.md` and `docs/architecture.md`, and dead ENOENT lock branches in `src/registry.ts`. `11-final` confirmed all five fixes with four of four sources and three new minors: an unpinned post-write fence, a spurious conflict when a peer chained from the compacted revision, and a registry-wide lock path missing from `docs/architecture.md`. All three are corrected with a fence regression that fails when the check is removed. Round `12-final` re-confirmed with four of four sources, no degradation and no findings on main `091d499`, native `e785953`, Fusion `b113212`, Bridge `3e99ec2` and Revmux `988904f`.

Strict ownership currently requires supported macOS GUI launchd and compiler prerequisites. Strict Fusion early-agreement mode is explicitly refused. Source installation uses the documented project-local Git dependency policy. No release, global installation, persistent daemon or PR merge is included.

## Next step

The confirmed revisions are pushed and CI is green; keep all PRs draft. No release, global installation, persistent daemon or PR merge is included. Preserve the explicit limitations that upstream Revmux Linux CI is waiting for maintainer approval and mixed/unknown legacy Fusion writers require reconciliation; process-group observations cannot be promoted to owned-tree proof.
