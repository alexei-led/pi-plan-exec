...
process.env.PI_AUTONOMOUS_SMOKE_CALLS = join(
  sandbox,
  'scripted-model-calls.jsonl',
);
const preload = join(sandbox, 'isolate.mjs');
await writeFile(
  preload,
  `import os from "node:os"; import { syncBuiltinESMExports } from "node:module"; os.homedir = () => ${JSON.stringify(sandbox)}; syncBuiltinESMExports();\n`,
);
...
const installedLock = JSON.parse(
  await readFile(join(project, 'node_modules/.package-lock.json'), 'utf8'),
);
for (const name of [
  'pi-subagents',
  ...
  assert.match(
    declared ?? '',
    /^\^\d+\.\d+\.\d+$/,
    `${name} must use a released registry pin`,
  );
  ...
  for (const source of [lock, installedLock]) {
    ...
    const [installedMajor, installedMinor, installedPatch] = String(
      entry.version,
    )
      .split('.')
      .map(Number);
    assert.ok(
      ...
      `${name} installed version must satisfy the declared range`,
    );
  }
}
...
const { registerSubagentRpcBridge } = await jiti.import(
  join(nativeRoot, 'src/extension/rpc.js'),
);
const { setChildSessionFactory, setChildSessionFactoryModule } =
  await jiti.import(join(nativeRoot, 'src/runs/shared/child-session.js'));
...
} = await jiti.import(join(project, 'src/bridge.ts'));
const { cancelOwnedProcess, observeOwnedProcess } = await jiti.import(
  join(project, 'src/owned-process.ts'),
);
const { PlanExecController } = await jiti.import(
  join(project, 'src/controller.ts'),
);
...
test('scripted model completes real controller, Bridge, owned-process workers, checks, review and promotion', {
  ...
  t.diagnostic(
    `Retained smoke evidence: ${sandbox}; model turns are scripted, not a live-LLM guarantee.`,
  );
  ...
  const inheritedHookEnvironment = {
    ...process.env,
    GIT_DIR: join(sandbox, 'canary-metadata'),
    GIT_WORK_TREE: join(sandbox, 'canary-worktree'),
    GIT_CONFIG_COUNT: '1',
    GIT_CONFIG_KEY_0: 'core.bare',
    GIT_CONFIG_VALUE_0: 'true',
    GIT_INDEX_FILE: join(sandbox, 'canary-index'),
  };
  const git = (cwd, ...args) =>
    execFileSync('git', args, {
      cwd,
      env: fixtureGitEnvironment(inheritedHookEnvironment),
      encoding: 'utf8',
  ...
  await assert.rejects(access(inheritedHookEnvironment.GIT_DIR), {
    code: 'ENOENT',
  });
  await assert.rejects(access(inheritedHookEnvironment.GIT_INDEX_FILE), {
    code: 'ENOENT',
  });
  ...
  const events = {
    ...
    emit(name, value) {
      if (name === 'plan-exec:bridge:v2:request' && value.method === 'spawn')
        requests.push(value);
  ...
  const agents = ['worker', 'reviewer'].map((name) => ({
    name,
    description: `Scripted smoke ${name}`,
    systemPrompt: '',
  ...
  setChildSessionFactoryModule(
    join(project, 'test/fixtures/autonomous-scripted-session.mjs'),
  );
  ...
  const executor = createSubagentExecutor({
    ...
    asyncByDefault: true,
    tempArtifactsDir: join(sandbox, 'artifacts'),
    getSubagentSessionRoot: () => join(sandbox, 'sessions'),
    expandTilde: (value) => value,
    discoverAgents: () => ({ agents }),
  });
  const nativeRpc = registerSubagentRpcBridge({
    events,
    state,
    asyncDirRoot: ASYNC_DIR,
    resultsDir: RESULTS_DIR,
    getContext: () => ctx,
    execute: (...args) => executor.executePublic(...args),
  });
  const bridgeRpc = registerPlanExecRpc(events, {
    timeoutMs: 15_000,
    journalPath: join(sandbox, 'bridge.sqlite'),
  });
    ...
    try {
      for (const request of requests) {
        const binding = {
          ...
          requestDigest: request.owner.requestDigest,
        };
        const terminal = async (operation) => {
          ...
          return (
            status.success &&
            hasTerminalOwnershipProof(status.data, runId, binding)
          );
        };
        let operation = await bridge.operation(
          request.operationId,
          request.owner,
        );
        if (!(await terminal(operation))) {
          ...
          do {
            await delay(100);
            operation = await bridge.operation(
              request.operationId,
              request.owner,
            );
        ...
        assert.ok(
          ...
          `Owned runtime cleanup proof missing: ${request.operationId}`,
        );
      }
      for (const directory of await readdir(localRoot).catch((error) => {
        if (error.code === 'ENOENT') return [];
        throw error;
      })) {
        const generation = join(localRoot, directory, 'generation-0');
        const binding = JSON.parse(
          await readFile(join(generation, 'binding.json'), 'utf8'),
        );
        let observation = await observeOwnedProcess(
          join(generation, 'owned-process'),
        );
        if (!hasOwnedProcessRetirementProof(observation, binding))
          observation = await cancelOwnedProcess(
            join(generation, 'owned-process'),
            { deadlineMs: 15_000, cancelled: true },
          );
        assert.ok(
          hasOwnedProcessRetirementProof(observation, binding) ||
            (observation.status === 'never-started' &&
              observation.proof?.kind === 'never-started' &&
              ['operationId', 'requestDigest', 'hostId', 'bootId'].every(
                (key) => observation.proof[key] === binding[key],
              )),
  ...
  assert.equal(
    capabilities.processTreeOwnership?.escapedDescendants,
    'best-effort',
  );
  ...
  const fusion = new Proxy(
    {},
    {
      get: () => () => {
        throw new Error('Smoke requires the configured single-subagent review');
      },
    },
  );
  ...
  while (run.status === 'running' && Date.now() < deadline) {
    ...
    const transition = {
      status: run.status,
      stage: run.stage,
      operation: run.activeOperation?.operationId,
      wakeReason: run.wakeReason,
      error: run.error,
    };
    if (JSON.stringify(transitions.at(-1)) !== JSON.stringify(transition))
      transitions.push(transition);
    if (
      run.tasks?.['1']?.state === 'retry_wait' ||
      (run.stageAttempts.comprehensive_review ?? 0) > 1
    )
      break;
    if (run.activeOperation || (run.nextAttemptAt ?? 0) > Date.now())
      await delay(100);
  }
  await writeFile(
    join(sandbox, 'evidence.json'),
    JSON.stringify(
      {
        model: 'scripted',
        pins: {
          piSubagents: manifest.devDependencies['pi-subagents'],
          bridge: manifest.devDependencies['@alexeiled/pi-subagents-bridge'],
          fusion: manifest.devDependencies['@alexeiled/pi-fusion'],
        },
        run,
        transitions,
      },
  ...
  assert.equal(
    await readFile(join(root, 'result.txt'), 'utf8'),
    'autonomous runtime smoke\n',
  );
  git(root, 'merge-base', '--is-ancestor', run.acceptedHead, 'HEAD');
  const calls = (await readFile(process.env.PI_AUTONOMOUS_SMOKE_CALLS, 'utf8'))
    .trim()
    .split('\n')
  ...
  for (const request of requests) {
    ...
    const status = await bridge.status(runId);
    assert.ok(
      hasTerminalOwnershipProof(status.data, runId, {
        operationId: request.operationId,
        requestDigest: request.owner.requestDigest,
      }),
    );
  ...
  for (const directory of localDirectories) {
    ...
    const intent = JSON.parse(
      await readFile(join(generation, 'intent.json'), 'utf8'),
    );
    if (
      JSON.stringify(intent.commands) ===
    ...
    const observation = await observeOwnedProcess(
      join(generation, 'owned-process'),
    );
  ...
  assert.ok(
    requiredCheckRuns >= 2,
    'Required checks must execute for candidate acceptance and final verification',
  );
});
