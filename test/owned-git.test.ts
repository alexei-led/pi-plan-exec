...
const controllerFixture = fileURLToPath(
  new URL('./fixtures/owned-git-controller.mjs', import.meta.url),
);
...
async function waitForLine(
  ...
  await waitFor(label, async () =>
    lineCount(await fileText(path)) >= expected ? true : undefined,
  );
}
...
async function terminateDetachedWriters(
  path: string,
  gate: string,
): Promise<void> {
...
async function createHookFixture(
  kind: 'post-checkout' | 'post-merge',
  plan: string,
) {
  const directory = await realpath(
    await mkdtemp(join(tmpdir(), 'owned-git-controller-')),
  );
...
async function releaseAndClean(
  fixture: Awaited<ReturnType<typeof createHookFixture>>,
  ...
  try {
    await waitForLine(
      fixture.writerDone,
      1,
      'detached Git hook writer retirement',
    );
  } catch {
    retired = false;
  }
  ...
  if (!retired)
    await terminateDetachedWriters(fixture.writerStarted, fixture.gate);
...
async function readControllerState(
  path: string,
): Promise<{ id: string; lane?: string; status: string; stage: string }> {
  return JSON.parse(await waitForFile(path, 'controller state')) as {
    id: string;
    lane?: string;
    status: string;
    stage: string;
...
async function assertSameOperationIntent(
  root: string,
  runId: string,
  operationId: string,
  before: string | undefined,
): Promise<string> {
  const directory = localOperationDirectory({
    journalRoot: join(root, '.git', 'plan-exec-local'),
    runId,
    operationId,
    authorization: { path: 'unused', stopGeneration: 0 },
  });
  const intent = await waitForFile(
    join(directory, 'intent.json'),
    'durable Git operation intent',
  );
...
async function waitForOperationEntry(
  path: string,
  operationId: string,
): Promise<void> {
  await waitFor('owned executor entry', async () => {
    ...
    return text?.split('\n').some((line) => {
      ...
      try {
        return (
          (JSON.parse(line) as { operationId?: string }).operationId ===
          operationId
        );
      } catch {
        return false;
...
nativeTest(
  'owned post-checkout survives controller death and reconciles without duplicate worktree creation',
  async () => {
    ...
    try {
      ...
      const controller = controllerProcess(
        'start',
        fixture.root,
        join(fixture.root, '.git', 'runs'),
        fixture.statePath,
        fixture.bridgeMarker,
        startReady,
        startEntry,
        'plan.md',
      );
      ...
      await waitFor('worktree HEAD and detached writer', async () => {
        const head = await git(required(lane), 'rev-parse', 'HEAD');
        return head.code === 0 &&
          (await fileText(fixture.writerStarted)) !== undefined
          ? true
          : undefined;
      });
      const operationId = `git:worktree:${lane}`;
      const intentDigest = await assertSameOperationIntent(
        fixture.root,
        started.id,
        operationId,
        undefined,
      ...
      const restarted = controllerProcess(
        'until-spawn',
        fixture.root,
        join(fixture.root, '.git', 'runs'),
        fixture.statePath,
      ...
      assert.equal(
        await assertSameOperationIntent(
          fixture.root,
          started.id,
          operationId,
          intentDigest,
      ...
      await waitForLine(
        fixture.writerDone,
        1,
        'post-checkout writer completion',
      );
      await childExit(restarted);
      assert.equal(lineCount(await fileText(fixture.hookInvocations)), 1);
      assert.equal(lineCount(await fileText(fixture.bridgeMarker)), 1);
      assert.equal(
        await gitOk(fixture.root, 'worktree', 'list', '--porcelain').then(
          (output) => output.includes(required(lane)),
        ),
        true,
      );
      assert.equal(
        await readFile(join(required(lane), 'owned-git-artifact'), 'utf8'),
        'owned by detached Git hook\n',
      );
      assert.equal(
        (
          await git(
      ...
      assert.equal(
        (await git(required(lane), 'check-ignore', '--', 'owned-git-artifact'))
          .code,
        0,
      );
    } finally {
...
nativeTest(
  'owned post-merge survives controller death and promotes the same accepted candidate',
  async () => {
    const fixture = await createHookFixture(
      'post-merge',
      '### Task 1: A\n- [x] A\n',
    ...
    try {
      ...
      await waitFor('promoted HEAD and detached writer', async () => {
        ...
        return head.stdout.trim() === candidate &&
          (await fileText(fixture.writerStarted)) !== undefined
          ? true
          : undefined;
      });
      ...
      const intentText = await waitForFile(
        join(
          localOperationDirectory({
            journalRoot: join(fixture.root, '.git', 'plan-exec-local'),
            runId: run.id,
            operationId,
            authorization: { path: 'unused', stopGeneration: 0 },
          }),
      ...
      assert.equal(
        (
          JSON.parse(intentText) as { commands?: string[][] }
        ).commands?.[0]?.includes('--no-overwrite-ignore'),
        true,
      );
      await terminate(controller);
      assert.equal(lineCount(await fileText(fixture.hookInvocations)), 1);
      ...
      assert.equal(lineCount(await fileText(fixture.hookInvocations)), 1);
      assert.equal(
        await readFile(join(fixture.root, 'owned-git-artifact'), 'utf8'),
        'owned by detached Git hook\n',
      );
      assert.equal(
        (
      ...
      assert.equal(
        (await git(fixture.root, 'check-ignore', '--', 'owned-git-artifact'))
          .code,
        0,
      );
    } finally {
