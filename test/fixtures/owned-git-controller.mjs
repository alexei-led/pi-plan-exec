...
if (
  ...
  throw new Error(
    'owned-git-controller requires mode, root, registry, session, state, marker, ready, executor entry, and value',
  );
}
...
const bridge = {
  ...
  async spawn(operationId, params) {
    await appendFile(
      bridgeMarkerPath,
      `${JSON.stringify({ operationId, cwd: params.cwd, lifetime: params.executionLifetime })}\n`,
    );
    return ok({
      runId: operationId,
  ...
  async operation(operationId) {
    return ok({ state: 'absent', operationId, replaySafe: true });
  },
  ...
  async stop(operationId) {
    return ok({ state: 'stopped', operationId });
  },
};
const fusion = {
  async start() {
...
const executeLocalCommands = async (cwd, commands, options) => {
  if (commands.length)
    await appendFile(
      executorEntryPath,
      `${JSON.stringify({ operationId: options.operationId, commands })}\n`,
    );
...
const writeState = async (run) => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      id: run.id,
      lane: run.lanePreparation?.cwd,
      status: run.status,
      stage: run.stage,
      outputPromotion: run.outputPromotion,
      archiveOperation: run.archiveOperation,
    }),
  );
