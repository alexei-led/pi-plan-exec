...
const [mode, directory, argument] = process.argv.slice(2) as [
  string,
  string,
  string,
];
  ...
  if (mode === 'hold') {
    ...
    if (argument === 'contender') {
      await assert.rejects(
        acquireLock(join(directory, 'registry.lock'), 1),
        LockTimeoutError,
      );
      send('contended');
    }
    ...
    const updated = await registry.updateIfCurrent(
      { ...run, branch: String(process.pid) },
      run.updatedAt,
    );
