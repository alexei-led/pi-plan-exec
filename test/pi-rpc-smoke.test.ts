...
test("real Pi RPC registers exec and renders isolated status without model dispatch", { timeout: 20_000 }, async (t) => {
...
`);
  ...
  child.stdout.on("data", (data: Buffer) => {
    ...
    for (;;) {
      ...
      if (value.type === "response" && value.id === "commands" && isRecord(value.data) && Array.isArray(value.data.commands)) {
        const commands = value.data.commands;
        commandsRegistered = ["exec", "goal"].every((name) =>
          commands.some((command: unknown) => isRecord(command) && command.name === name));
        child.stdin.write(`${JSON.stringify({ id: "status", type: "prompt", message: "/exec status" })}\n`);
      }
      if (value.type === "extension_ui_request" && typeof value.message === "string" && /No .*runs/i.test(value.message)) statusRendered = true;
