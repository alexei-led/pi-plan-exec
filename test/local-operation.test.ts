...
function ownedDirectory(
  ...
  return join(
    generation === undefined
      ? localOperationDirectory(options)
      : localOperationDirectory({
          ...options,
          authorization: {
            path: options.authorization?.path ?? '',
...
async function untilFile(path: string): Promise<string> {
  ...
  for (;;) {
    try {
      ...
      if (Date.now() > end) throw new Error(`File absent: ${path}`);
    }
    await delay(20);
  }
}
...
`;
  const child = spawn(
    process.execPath,
    ['--import', 'jiti/register', '--input-type=module', '-e', script],
    {
...
async function clean(cwd: string, options: LocalOperationOptions) {
  ...
  if (await exists(owned)) {
    const observation = await cancelOwnedProcess(owned, {
      deadlineMs: 10_000,
      cancelled: true,
    });
    assert.ok(
...
test('lost result reply and concurrent attachment execute the unbounded batch exactly once', async () => {
  ...
  try {
    const commands = [
      [process.execPath, '-e', "require('fs').appendFileSync('count','x')"],
    ];
    await Promise.all([
    ...
    try {
      ...
      if (oldAmbient === undefined)
        delete process.env.PI_PLAN_EXEC_REPLAY_AMBIENT;
      else process.env.PI_PLAN_EXEC_REPLAY_AMBIENT = oldAmbient;
    }
    ...
    await assert.rejects(
      runLocalOperation(cwd, commands, { ...options, candidate: 'different' }),
      LocalOperationUnknownError,
    );
  } finally {
...
test('controller death leaves one command that a new controller adopts', async () => {
  ...
  const commands = [
    [
      process.execPath,
      '-e',
      "require('fs').appendFileSync('count','x'); setTimeout(()=>{},1200)",
    ],
  ];
...
test('local worker excludes inherited Git selectors and injected config from its immutable environment', async () => {
  ...
  try {
    const gitKeys = Object.keys(inherited).filter((key) =>
      key.startsWith('GIT_'),
    );
    const commands = [
      [
        process.execPath,
        '-e',
        `require('fs').writeFileSync('environment.json',JSON.stringify({git:${JSON.stringify(gitKeys)}.filter(key=>process.env[key]!==undefined),canary:process.env.PI_LOCAL_ENV_CANARY}))`,
      ],
    ];
    await runLocalOperation(cwd, commands, options);
    assert.deepEqual(
      JSON.parse(await readFile(join(cwd, 'environment.json'), 'utf8')),
      { git: [], canary: 'preserved' },
    );
    ...
    assert.equal(
      createHash('sha256')
        .update(await readFile(join(directory, 'intent.json')))
        .digest('hex'),
...
test('fast detached descendant survives its batch wrapper', async () => {
  ...
  try {
    const descendant =
      "setTimeout(()=>require('fs').writeFileSync('descendant-done','yes'),600)";
...
test('stop retires the owned process group and fences later commands', async () => {
  ...
  const commands = [
    [
      process.execPath,
      '-e',
      "require('fs').writeFileSync('started','yes');setInterval(()=>{},1000)",
    ],
    [process.execPath, '-e', "require('fs').writeFileSync('late','bad')"],
  ];
  try {
...
test('stop before launch is durable and has no command side effects', async () => {
  ...
  try {
    ...
    await assert.rejects(
      runLocalOperation(
        cwd,
        [[process.execPath, '-e', "require('fs').writeFileSync('bad','bad')"]],
        options,
      ),
      LocalOperationCancelledError,
    );
  ...
  try {
    const commands = [
      [
        process.execPath,
        '-e',
        "require('fs').appendFileSync('count','x');process.exit(7)",
      ],
      [process.execPath, '-e', "require('fs').writeFileSync('late','bad')"],
    ];
    await assert.rejects(
    ...
    assert.equal(await readFile(join(cwd, 'count'), 'utf8'), 'x');
    await assert.rejects(readFile(join(cwd, 'late')), { code: 'ENOENT' });
    await runLocalOperation(
      cwd,
      [[process.execPath, '-e', "require('fs').writeFileSync('retry','yes')"]],
      { ...options, operationId: 'checks:retry-1' },
    );
...
test('an offline stop is honored by the next dispatcher after controller death', async () => {
  ...
  await durableJson(authorizationPath, {
    id: 'run',
    status: 'running',
    stopGeneration: 0,
  });
...
test('authorization lookup failure preserves the running operation for reconciliation', async () => {
  ...
  const commands = [
    [
      process.execPath,
      '-e',
      "require('fs').writeFileSync('started','yes'); setTimeout(()=>{},500)",
    ],
  ];
  try {
    const running = runLocalOperation(cwd, commands, options);
  ...
  try {
    ...
    const failed = assert.rejects(
      ...
      (error) =>
        ...
        error.message.includes('lost-result-after-exit'),
    );
    await untilFile(join(cwd, 'started'));
    const launch = JSON.parse(
      await readFile(join(ownedDirectory(options), 'launch.json'), 'utf8'),
    );
    ...
    await runLocalOperation(
      cwd,
      [[process.execPath, '-e', "require('fs').writeFileSync('retry','yes')"]],
      { ...options, operationId: 'checks:after-monitor-death' },
    );
    assert.equal(await readFile(join(cwd, 'retry'), 'utf8'), 'yes');
  } finally {
    await clean(cwd, options);
...
test('a restarted stop discovers and retires local descendants after both monitors die', async () => {
  ...
  const commands = [
    [
      process.execPath,
      '-e',
      "require('fs').writeFileSync('child-pid',String(process.pid));setInterval(()=>{},1000)",
    ],
  ];
  const controller = await controllerFor(cwd, commands, options);
  try {
    ...
    await killController(controller);
    process.kill(launch.pid, 'SIGKILL');
    let cleanup = await cancelActiveLocalOperations(
    ...
    while (cleanup.pending && Date.now() < deadline) {
      await delay(100);
      cleanup = await cancelActiveLocalOperations(
        options.activeDirectory,
        options.runId,
        1,
      );
    ...
    assert.equal(
      cleanup.pending,
      false,
      cleanup.reason ?? 'Local cleanup did not retire the owned scope.',
    );
    assert.equal(
      await hasActiveLocalOperations(options.activeDirectory),
      false,
    );
    assert.throws(() => process.kill(childPid, 0), { code: 'ESRCH' });
    await runLocalOperation(
      cwd,
      [[process.execPath, '-e', "require('fs').writeFileSync('retry','safe')"]],
...
test('a new stop generation starts only after prior ownership retires', async () => {
  ...
  const commands = [
    [
      process.execPath,
      '-e',
      "require('fs').appendFileSync('count','x');setTimeout(()=>{},300)",
    ],
  ];
  try {
    await runLocalOperation(cwd, commands, options);
    ...
    const successor = {
      ...options,
      authorization: { path: authorizationPath, stopGeneration: 1 },
    };
...
test('an empty successor batch retires the prior generation before returning', async () => {
  ...
  try {
    await untilFile(join(cwd, 'started'));
    assert.equal(
      (await observeOwnedProcess(join(directory, 'owned-process'))).status,
      'running',
    );
    const successor = {
      ...options,
      authorization: { path: join(cwd, 'run.json'), stopGeneration: 1 },
    };
...
test('a new stop generation fences an empty interrupted generation before starting', async () => {
  ...
  try {
    ...
    await durableJson(join(cwd, 'run.json'), {
      id: 'run',
      status: 'running',
      stopGeneration: 1,
    });
    ...
    assert.ok(await readFile(join(directory, 'stop.json'), 'utf8'));
    await assert.rejects(
      runLocalOperation(cwd, [[process.execPath, '-e', '0']], options),
      LocalOperationCancelledError,
    );
    await runLocalOperation(cwd, [[process.execPath, '-e', '0']], successor);
  } finally {
...
test('abandoned intent and cancellation publications cannot park an unstarted generation', async () => {
  ...
  try {
    await mkdir(directory, { recursive: true });
    for (const name of ['intent', 'stop'])
      await writeFile(
        join(
          directory,
          `${name}.json.00000000-0000-0000-0000-000000000000.tmp`,
        ),
        '{',
      );
    const successor = {
...
for (const publication of [
  ...
  test(`a fenced incomplete generation cannot launch ${publication}`, async () => {
    ...
    const resume =
      publication === 'after successor starts'
        ? join(cwd, 'resume-old')
        : join(directory, 'stop.json');
    const stale = await controllerFor(cwd, commands, options, {
      gate: { destination: join(directory, 'intent.json'), ready, resume },
      cancelled: true,
    });
    const staleExit = new Promise<number | null>((resolve) =>
      stale.once('exit', resolve),
    );
    let current: ReturnType<typeof spawn> | undefined;
    try {
      await untilFile(ready);
      await assert.rejects(readFile(join(directory, 'intent.json')), {
        code: 'ENOENT',
      });
      await durableJson(authorizationPath, {
        id: 'run',
      ...
      current = await controllerFor(
        cwd,
        commands,
        successor,
        publication === 'while the fence is published'
          ? {
              gate: {
                destination: join(directory, 'stop.json'),
                ready: join(cwd, 'fence-published'),
                resume: join(directory, 'binding.json'),
                after: true,
              },
            }
          : undefined,
      );
      const currentExit = new Promise<number | null>((resolve) =>
        current?.once('exit', resolve),
      );
      await Promise.race([
        untilFile(join(cwd, 'count')),
        currentExit.then((code) => assert.equal(code, 0)),
      ]);
      if (publication === 'after successor starts')
      ...
      await cancelOwnedProcess(
        join(localOperationDirectory(successor), 'owned-process'),
        { deadlineMs: 10_000, cancelled: true },
      );
      await clean(cwd, options);
    }
  });
...
for (const artifact of [
  ...
  test(`an incomplete generation with ${artifact} remains fenced`, async () => {
    ...
    try {
      ...
      } else if (artifact === 'owned-process')
      ...
      await assert.rejects(
        runLocalOperation(cwd, [[process.execPath, '-e', '0']], successor),
        LocalOperationUnknownError,
      );
      await assert.rejects(
        readFile(join(localOperationDirectory(successor), 'intent.json')),
        { code: 'ENOENT' },
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
