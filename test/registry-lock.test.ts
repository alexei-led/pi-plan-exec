...
function worker(_t: TestContext, args: string[], env?: NodeJS.ProcessEnv) {
    ...
    async event(name: string): Promise<unknown> {
      while (!events.length) {
        await Promise.race([
          ...
          exited.then(() => {
            throw new Error(`Worker exited before ${name}: ${output}`);
          }),
        ]);
        waiting = undefined;
      }
...
async function directory(_t: TestContext): Promise<string> {
  const path = await mkdtemp(join(tmpdir(), 'registry-kernel-lock-test-'));
  onTestFinished(() => rm(path, { recursive: true, force: true }));
  return path;
}

test('kernel lock survives old metadata and only releases on owner death', {
  timeout: 15_000,
}, async (t) => {
...
test('releasing an old handle twice cannot release its successor', {
  timeout: 15_000,
}, async (t) => {
  const root = await directory(t);
  const path = join(root, 'held.lock');
...
test('two crash recoverers admit only one run while the first pauses before publication', {
  timeout: 15_000,
}, async (t) => {
  const root = await directory(t);
  ...
  const refused = (await second.event('result')) as {
    created: boolean;
    error: string;
  };
...
test('cross-process compare-and-set applies exactly one shared revision', {
  timeout: 15_000,
}, async (t) => {
  const root = await directory(t);
  ...
  const crashed = worker(t, [
    'hold',
    join(root, '.locks', `${run.id}.record.lock`),
  ]);
  await crashed.event('held');
...
test('cold native compilation needs no Node headers or checkout artifacts', {
  timeout: 15_000,
}, async (t) => {
  const root = await directory(t);
  const child = worker(t, ['hold', join(root, 'lock')], {
    ...process.env,
    TMPDIR: root,
  });
  ...
  const cachePath = join(
    root,
    `pi-plan-exec-registry-lock-${process.getuid?.()}`,
  );
  const cache = await stat(cachePath);
  assert.equal(cache.mode & 0o777, 0o700);
  const binaries = (await readdir(cachePath)).filter((name) =>
    name.endsWith('.node'),
  );
  assert.equal(binaries.length, 1);
  assert.equal(
    (await stat(join(cachePath, required(binaries[0])))).mode & 0o777,
    0o600,
  );
});

test('removing a run preserves the lock inodes seen by waiting writers', {
  timeout: 15_000,
}, async (t) => {
  const root = await directory(t);
  const registry = new RunRegistry(root);
  ...
  assert.deepEqual(
    after.map((value) => value.ino),
    before.map((value) => value.ino),
  );
  await assert.rejects(
    registry.updateIfCurrent(run, run.updatedAt),
    /run not found/,
  );
  assert.equal(await registry.get(run.id), undefined);
});

test('distributed TypeScript lock module works outside the checkout with a fresh compiler cache', {
  timeout: 15_000,
}, async (t) => {
  const root = await directory(t);
  ...
  await copyFile(
    new URL('../src/registry-lock.ts', import.meta.url),
    join(consumer, 'registry-lock.ts'),
  );
  await writeFile(
