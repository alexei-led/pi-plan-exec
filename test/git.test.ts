...
test('path containment does not confuse sibling repository names', () => {
  ...
  assert.equal(
    isPathWithin('/repo', '/repo-other/docs/plans/example.md'),
    false,
  );
  assert.equal(isPathWithin('/repo', '/other/example.md'), false);
});

test('accepts a symlink alias of a registered worktree with a newline in its path', async (_t) => {
...
test('rejects a directory that is not a registered worktree', async (_t) => {
  const target = await mkdtemp(resolve(tmpdir(), 'exec-git-'));
  onTestFinished(() => rm(target, { recursive: true, force: true }));
  const run = async () => ({
...
test('plans execution worktree paths without running Git before durable intent exists', () => {
  ...
  const worktree = executionWorktreePath(repositoryRoot, 'example');
