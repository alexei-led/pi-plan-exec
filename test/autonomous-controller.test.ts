...
const command: RunCommand = async (program, args, cwd) => {
  try {
    const r = await execute(program, args, { cwd });
    return { ...r, code: 0 };
  } catch (error) {
    const r = error as { stdout?: string; stderr?: string; code?: number };
    return {
      stdout: r.stdout ?? '',
      stderr: r.stderr ?? '',
      code: r.code ?? 1,
...
const ok = (data: Record<string, unknown>): BridgeResult => ({
  success: true,
  data,
});
...
function observedWorkerProof(id: string, digest: string) {
  return {
    ...
    observedAt: Date.now(),
    callerBinding: { operationId: id, requestDigest: digest },
    instances: [],
  };
}
...
class Worker {
  ...
  async capabilities() {
    return {
      ...
      executionLifetimeModes: ['unbounded' as const, 'bounded' as const],
  ...
  async spawn(
    ...
    if (!this.launches.some((item) => item.id === id))
  ...
  async status(id: string): Promise<BridgeResult> {
    if (this.failStatus)
      return { success: false, error: { message: 'status unavailable' } };
    return ok({
      state: this.state,
  ...
  async result() {
    await writeFile(this.resultPath, JSON.stringify({ result: this.output }));
    return ok({ resultPath: this.resultPath });
  }
  async adopt() {
    return ok({});
  }
...
class GuidingWorker extends Worker {
  ...
  async diagnoseOperation(
    ...
    if (this.guidanceCalls.length === 1)
      return {
        success: false,
        error: { code: 'timeout', message: 'guidance reply lost' },
      };
    return ok({
      operationId,
...
class DurableReviewBackend {
  calls: {
    ...
    lifetime: ExecutionLifetime | undefined;
    digest: string | undefined;
    context: { cwd: string; reviewedCommit: string } | undefined;
  }[] = [];
  ...
  async start(
    ...
    if (this.cancelled.has(id))
      return {
        success: false,
        error: { code: 'cancelled', message: 'launch fenced' },
      };
    if (this.calls.length === 1)
      return {
        success: false,
        error: { code: 'timeout', message: 'start never reached admission' },
      };
  ...
  async status(runId?: string, operationId?: string): Promise<BridgeResult> {
    const id = runId ?? required(operationId);
    if (this.lookupUnavailable)
      return {
        success: false,
        error: { code: 'not_found', message: 'not authoritative' },
      };
    if (this.cancelled.has(id))
      return ok({
        operationId: id,
    ...
    if (!this.children.has(id))
      return ok({ operationId: id, state: 'absent', replaySafe: true });
    const request = required(this.calls.find((call) => call.id === id));
    return ok({
      runId: id,
      operationId: id,
      phase: 'panel',
      ...
      effectiveExecutionLifetime: request.lifetime,
    });
  }
  async cancel(runId?: string, operationId?: string) {
    ...
    return ok({
      ...
      state: 'cancelled',
      cancellationRequested: true,
      neverStarted: !this.children.has(id),
      replaySafe: false,
    });
  }
  result(runId?: string, operationId?: string) {
    return this.status(runId, operationId);
  }
  adopt(runId: string) {
...
async function fixture(
  ...
  content = '### Task 1: A\n- [ ] A\n',
  subdirectory = '',
  planDirectory = subdirectory,
  trackedPlan = true,
) {
  const root = await realpath(
    await mkdtemp(join(tmpdir(), 'autonomous-controller-')),
  );
  ...
  const run = await registry.create({
    ...
    reviewFindings: [],
    unresolvedFindings: [],
    config: { ...DEFAULT_FROZEN_RUN_CONFIG, retryDelayMs: 10 },
    skippedStages: [],
    branchRebindings: [],
  });
  return {
...
async function due(registry: RunRegistry, run: PlanExecRun) {
  return registry.update({
    ...
    nextAttemptAt: 0,
    ...(run.activeOperation
      ? { activeOperation: { ...run.activeOperation, launchStartedAt: 0 } }
      : {}),
  });
}
...
for (const change of [
  ...
  test(`approved dependency change ${JSON.stringify(change.before)} to ${JSON.stringify(change.after)} recomputes readiness`, async (t) => {
    const content = (dependencies: number[]) =>
    ...
    let run = await f.registry.update({
      ...
      tasks: {
        '1': {
          ...
          reason: 'Provider credentials remain unavailable',
        },
        '2': {
          taskId: 2,
          dependsOn: change.before,
          state: change.before.length ? 'waiting_dependency' : 'ready',
          attempts: 3,
          lastScheduledAt: 1,
          laneCwd: f.root,
          laneBranch: 'feature',
...
for (const trackedPlan of [true, false]) {
  test(`approved dependency removal completes in a fresh nested lane with ${trackedPlan ? 'tracked' : 'untracked'} baseline plan`, async (t) => {
    ...
    await f.git('add', 'packages/api/package.txt');
    await f.git('commit', '-m', 'nested package');
    vi.useFakeTimers({ toFake: ['Date'] });
    vi.setSystemTime(Date.now());
    ...
    const approvedContent = original
      .replace('dependsOn: [1]', 'dependsOn: []')
      .replace('[ ] A', '[x] A');
    ...
    run = await f.controller.resume(
      ...
      'session',
      true,
      (await readPlan(f.planPath)).hash,
    );
    ...
    assert.equal(
      run.activeOperation?.taskId,
      2,
      run.error ?? 'B did not launch',
    );
    assert.ok(run.worktreeCwd.endsWith('/packages/api'));
    assert.equal(
      (await readPlan(planPathOf(run))).tasks[0]?.unchecked.length,
      1,
    );
    await assert.rejects(readFile(join(run.worktreeCwd, 'partial.txt')), {
      code: 'ENOENT',
    });
    await writeFile(
      planPathOf(run),
      (await readFile(planPathOf(run), 'utf8')).replace('[ ] B', '[x] B'),
    );
    assert.equal(
      (await command('git', ['add', '--all'], run.worktreeCwd)).code,
      0,
    );
    assert.equal(
      (
        await command(
          'git',
          ['commit', '-m', 'B under approved dependencies'],
          run.worktreeCwd,
        )
      ).code,
      0,
    ...
    assert.equal(
      run.tasks?.['2']?.state,
      'accepted',
      run.tasks?.['2']?.reason ?? 'B was not accepted',
    );
    ...
    assert.equal(
      await readFile(join(sourceCwd, 'partial.txt'), 'utf8'),
      'A private partial work\n',
    );
  });
}
...
for (const outcome of ['failed', 'complete', 'superseded']) {
  test(`interrupted approved plan publication reconciles its ${outcome} operation before inspecting its dirty lane`, async (t) => {
    ...
    assert.ok(damagedLane, 'damaged lane path missing');
    ...
    const executor: typeof runCommands = async (cwd, commands, options) => {
      if (
        options.operationId.startsWith('plan:publish:') &&
        cwd === damagedLane
      ) {
        publicationIds.push(options.operationId);
        if (publicationIds.length === 1) {
          if (outcome === 'failed')
            await writeFile(join(cwd, 'plan.md'), 'partial publication');
          else await f.localExecutor(cwd, commands, options);
          throw new LocalOperationUnknownError(
            'Publication process ownership remains unresolved.',
          );
        }
        if (outcome === 'failed')
          throw new LocalOperationFailedError(
            'Publication failed after confirmed process-tree exit.',
          );
      }
    ...
    for (let step = 0; step < 3 && publicationIds.length === 0; step++)
    ...
    if (outcome === 'superseded') {
      await writeFile(
        f.planPath,
        (await readFile(f.planPath, 'utf8')).replace(
          'Task 2: B',
          'Task 2: B updated',
        ),
      );
      run = await controller.resume(
        run.id,
        'session',
        true,
        (await readPlan(f.planPath)).hash,
    ...
    if (outcome !== 'complete')
      assert.notEqual(run.lanePreparation?.cwd, damagedLane);
    ...
    assert.equal(
      run.activeOperation?.taskId,
      2,
      run.error ?? 'B did not launch after publication recovery',
    );
    if (outcome === 'failed')
      assert.equal(
        await readFile(join(damagedLane, 'plan.md'), 'utf8'),
        'partial publication',
      );
    if (outcome === 'superseded') {
      assert.equal(
        (await readPlan(join(damagedLane, 'plan.md'))).tasks[1]?.title,
        'B',
      );
      assert.equal(
        (await readPlan(planPathOf(run))).tasks[1]?.title,
        'B updated',
      );
    }
...
`,
    ...
    const publisher = new URL('../src/plan-publisher.mjs', import.meta.url)
      .pathname;
    const failed = await command(
      process.execPath,
      ['--import', injector, publisher, payloadPath, digest],
      f.root,
    );
    assert.notEqual(failed.code, 0);
    assert.equal(await readFile(f.planPath, 'utf8'), original);
    const temporary = (await readdir(f.root)).find((name) =>
      name.startsWith('.plan-exec-'),
    );
    assert.ok(temporary);
    assert.equal(
      await readFile(join(f.root, temporary), 'utf8'),
      content.slice(0, 20),
    );
  });
}
...
test('approved dependency addition prevents acceptance of an already active B', async (t) => {
  ...
  await writeFile(
    f.planPath,
    original
      .replace('dependsOn: []', 'dependsOn: [1]')
      .replace('[ ] B', '[x] B'),
  );
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'B with new dependency');
  f.worker.state = 'complete';
  f.worker.proof = true;
  run = await f.controller.resume(
    run.id,
...
for (const initialSnapshot of [false, true]) {
  test(`default untracked independent lane excludes failed task checkbox claims ${initialSnapshot ? 'with' : 'without'} an initial snapshot`, async (t) => {
    ...
    let run = initialSnapshot
      ? await f.registry.update({
          ...f.run,
          initialPlan: { hash: required(f.run.planHash), content },
        })
      : f.run;
    run = await f.controller.tick(run.id, 'session');
    ...
    if (!initialSnapshot) {
      ...
      await writeFile(
        planPathOf(run),
        (await readFile(planPathOf(run), 'utf8')).replace(
          '[ ] Existing work',
          '[x] Existing work',
        ),
      );
      for (const args of [
        ['add', 'plan.md'],
        ['commit', '-m', 'verify uncertain legacy work'],
      ])
        assert.equal((await command('git', args, run.worktreeCwd)).code, 0);
      f.worker.state = 'complete';
      f.worker.proof = true;
    ...
    assert.equal(
      run.tasks?.['4']?.state,
      'accepted',
      run.tasks?.['4']?.reason ?? 'Existing work parked',
    );
...
for (const occupied of [true, false]) {
  for (const subdirectory of ['', 'packages/api']) {
    test(`adoption and fresh lanes agree on renamed accepted items and new checked tasks with an ${occupied ? 'occupied' : 'unoccupied'} ${subdirectory ? 'nested' : 'root'} lane`, async (t) => {
      ...
      let run = await f.registry.update({
        ...
        tasks: {
          '1': {
            ...
            attempts: 1,
            lastScheduledAt: 1,
            acceptedCommit: baseline,
            baselineCommit: baseline,
            laneCwd: f.run.worktreeCwd,
            laneBranch: 'feature',
          },
          '2': {
            taskId: 2,
            dependsOn: [1],
            state: occupied ? 'waiting_external' : 'accepted',
            attempts: 1,
            nextAttemptAt: Date.now() + 60_000,
            laneCwd: f.run.worktreeCwd,
            laneBranch: 'feature',
      ...
      if (occupied)
        await writeFile(
          join(f.root, 'partial.txt'),
          'Blocked task partial code',
        );
      run = await f.controller.resume(
        run.id,
        'session',
      ...
      for (const taskId of [3, 1]) {
        ...
        for (let step = 0; step < 12 && !run.activeOperation; step++) {
          if (run.lanePreparation) lanes.add(run.lanePreparation.cwd);
          run = await f.controller.tick(
            (await due(f.registry, run)).id,
            'session',
          );
        }
        assert.equal(
          run.activeOperation?.taskId,
          taskId,
          run.error ?? 'Expected task did not launch',
        );
        assert.notEqual(run.worktreeCwd, f.run.worktreeCwd);
        const item = taskId === 3 ? 'New work' : 'Add REST API';
        assert.deepEqual(
          (await readPlan(planPathOf(run))).tasks[taskId - 1]?.unchecked,
          [item],
        );
        await writeFile(
          planPathOf(run),
        ...
        run = await f.controller.tick(run.id, 'session');
        assert.equal(
          run.tasks?.[String(taskId)]?.state,
          'accepted',
          run.tasks?.[String(taskId)]?.reason ?? 'Candidate rejected',
        );
      }
      assert.equal(await readFile(f.planPath, 'utf8'), approved);
      if (occupied)
        assert.equal(
          await readFile(join(f.root, 'partial.txt'), 'utf8'),
          'Blocked task partial code',
        );
    });
  }
...
test('initial untracked plan publication recovers a partial write with its original authorized checkbox facts', async (t) => {
  ...
  const lanes = new Set<string>();
  onTestFinished(async () => {
    for (const lane of lanes) await rm(lane, { recursive: true, force: true });
  });
  ...
  const executor: typeof runCommands = async (cwd, commands, options) => {
    if (
      ...
      if (publicationIds.length === 1) {
        ...
        throw new LocalOperationUnknownError(
          'Initial publication ownership is unresolved.',
        );
      }
      throw new LocalOperationFailedError(
        'Initial publication exited after a partial write.',
      );
    }
    return f.localExecutor(cwd, commands, options);
  };
  ...
  assert.equal(
    run.activeOperation?.taskId,
    2,
    run.error ?? 'Initial plan recovery did not launch A',
  );
  assert.equal(await readFile(planPathOf(run), 'utf8'), original);
  assert.equal(await readFile(f.planPath, 'utf8'), original);
  assert.equal(
    await readFile(join(required(damagedLane), 'plan.md'), 'utf8'),
    '### Task 1: Existing\n- [x] Existing\n',
  );
});
...
test('independent B publishes normalized facts for a reopened A that is waiting on a prerequisite', async (t) => {
  ...
  const approved = original.replace(
    'dependsOn: []\n- [x] Old work',
    'dependsOn: [1]\n- [x] New work',
  );
  await writeFile(f.planPath, approved);
  ...
  assert.equal(run.tasks?.['2']?.state, 'waiting_dependency');
  assert.deepEqual((await readPlan(planPathOf(run))).tasks[1]?.unchecked, [
    'New work',
  ]);
  await writeFile(
    planPathOf(run),
  ...
  for (const args of [
    ['add', 'plan.md'],
    ['commit', '-m', 'B keeps blocked A pending'],
  ])
    assert.equal((await command('git', args, run.worktreeCwd)).code, 0);
  f.worker.state = 'complete';
  f.worker.proof = true;
...
for (const reapprove of [false, true]) {
  test(`tracked dirty initial checkbox facts survive independent B publication ${reapprove ? 'after reapproval' : 'without reapproval'}`, async (t) => {
    ...
    let run = await f.controller.start({
      cwd: f.root,
      planPath: f.planPath,
      useWorktree: false,
      sessionId: 'session',
    });
    for (let step = 0; step < 10 && !run.activeOperation; step++)
      run = await f.controller.tick(run.id, 'session');
    assert.equal(run.activeOperation?.taskId, 2);
    ...
    if (reapprove) {
      await writeFile(
        f.planPath,
        authorized.replace('Task 3: B', 'Task 3: B renamed'),
      );
      run = await f.controller.resume(
        run.id,
        'session',
    ...
    for (const args of [
      ['add', 'plan.md'],
      ['commit', '-m', 'B retains initially authorized A completion'],
    ])
      assert.equal((await command('git', args, run.worktreeCwd)).code, 0);
    f.worker.state = 'complete';
    f.worker.proof = true;
    ...
    await writeFile(
      planPathOf(run),
      `${await readFile(planPathOf(run), 'utf8')}### Task 4: Later\ndependsOn: []\n- [ ] Later\n`,
    );
    run = await f.controller.resume(
      run.id,
      'session',
      true,
      (await readPlan(planPathOf(run))).hash,
    );
    assert.equal(run.tasks?.['3']?.state, 'accepted');
    assert.equal(
      await readFile(join(f.root, 'blocked.txt'), 'utf8'),
      'preserved partial code',
    );
  });
}
...
test('approved new unchecked work reopens an accepted task without erasing its history', async (t) => {
  ...
  assert.deepEqual((await f.registry.get(run.id))?.approvedPlan, {
    hash: required(run.planHash),
    content,
  });
  await assert.rejects(
    f.registry.update({
      ...run,
      approvedPlan: { hash: required(run.planHash), content: 'invalid plan' },
    }),
...
test('explicit approval must match the current plan snapshot', async (t) => {
  ...
  const run = await f.controller.resume(
    f.run.id,
    'session',
    true,
    f.run.planHash,
...
test('approved structure rotates a stale existing task lane without overwriting it', async (t) => {
  ...
  assert.equal(
    await readFile(join(oldLane, 'partial-b.txt'), 'utf8'),
    'B preserved work\n',
  );
  await assert.rejects(readFile(join(run.worktreeCwd, 'partial-b.txt')), {
    code: 'ENOENT',
  });
});
...
test('unapproved structure drift in an alternate task lane still pauses', async (t) => {
  ...
  onTestFinished(() => rm(lane, { recursive: true, force: true }));
  ...
  let run = await f.registry.update({
    ...f.run,
    initialPlan: {
      hash: required(f.run.planHash),
      content: await readFile(f.planPath, 'utf8'),
    },
    tasks: {
      '1': {
        taskId: 1,
...
test('removing a dependency retains the task retry deadline and history', () => {
  ...
  for (const external of [false, true]) {
    ...
    assert.equal(
      reconciled['2']?.state,
      external ? 'waiting_external' : 'retry_wait',
    );
...
for (const planDirectory of ['packages/api', 'docs/plans']) {
  test(`nested execution keeps the worker cwd with plan in ${planDirectory}`, async (t) => {
    ...
    let run = await f.registry.update({
      ...f.run,
      progressPath: join(cwd, 'progress.txt'),
    });
    run = await f.controller.tick(run.id, 'session');
    ...
    await writeFile(required(run.progressPath), 'task progress\n');
    f.worker.state = 'complete';
    f.worker.proof = true;
    run = await f.controller.tick(run.id, 'session');
    assert.equal(
      run.tasks?.['1']?.state,
      'accepted',
      run.error ?? 'nested candidate rejected',
    );
    ...
    assert.equal(
      run.status,
      'completed',
      run.error ?? run.wakeReason ?? 'nested run did not complete',
    );
    assert.equal(run.worktreeCwd, cwd);
    assert.ok(f.worker.launches.every((launch) => launch.params.cwd === cwd));
    assert.match(
      await readFile(join(dirname(f.planPath), 'completed/plan.md'), 'utf8'),
      /\[x\] A/,
    );
...
test('nested independent task lane preserves the cwd and excludes partial work', async (t) => {
  ...
  assert.equal(
    run.activeOperation?.taskId,
    2,
    run.error ?? 'independent task did not launch',
  );
  ...
  assert.equal(
    await readFile(join(original, 'partial.txt'), 'utf8'),
    'preserved A\n',
  );
  await writeFile(planPathOf(run), content.replace('[ ] B', '[x] B'));
  assert.equal(
    (await command('git', ['add', 'plan.md'], run.worktreeCwd)).code,
    0,
  );
  assert.equal(
    (
      await command(
        'git',
        ['commit', '-m', 'independent nested candidate'],
        run.worktreeCwd,
      )
    ).code,
    0,
  ...
  assert.equal(
    run.tasks?.['2']?.state,
    'accepted',
    run.error ?? 'nested lane candidate rejected',
  );
  assert.equal(run.tasks?.['1']?.state, 'waiting_external');
});
...
test('plan drift after proven exit retains task recovery evidence across repair and resume', async (t) => {
  ...
  for (let step = 0; step < 4 && !run.activeOperation; step++)
    run = await f.controller.tick(run.id, 'session');
...
test('status uncertainty beyond diagnostic burst preserves one writer and automatic wake', async (t) => {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    assert.ok(required(run.nextAttemptAt) > Date.now());
  }
  assert.equal(f.worker.launches.length, 1);
...
test('external prerequisite A preserves its lane while B completes and C waits until automatic recovery', async (t) => {
  ...
  await f.git('commit', '-m', 'fixture ignored artifact policy');
  await writeFile(
    join(f.root, 'private-artifact'),
    'private bytes stay untouched',
  );
  let run = await f.controller.tick(f.run.id, 'session');
  ...
  assert.equal(run.status, 'running');
  assert.equal(run.tasks?.['1']?.state, 'waiting_external');
  assert.deepEqual(run.tasks?.['1']?.externalPrerequisite, {
    kind: 'credentials',
    source: 'worker',
    evidence: 'provider returned HTTP 401 invalid_token',
  });
  ...
  onTestFinished(() => rm(lane, { recursive: true, force: true }));
  for (let attempt = 0; attempt < 8 && !run.activeOperation; attempt++)
    run = await f.controller.tick(run.id, 'session');
  assert.equal(run.activeOperation?.taskId, 2);
  ...
  assert.equal(run.tasks?.['3']?.state, 'waiting_dependency');
  const acceptedB = required(run.acceptedHead);
  vi.advanceTimersByTime(
    required(run.tasks?.['1']?.nextAttemptAt) - Date.now(),
  );
  ...
  onTestFinished(() => rm(recoveryLane, { recursive: true, force: true }));
  ...
  const source = required(run.tasks?.['1']?.recoverySource);
  assert.equal(source.cwd, f.root);
  assert.equal(
    await readFile(join(f.root, 'partial.txt'), 'utf8'),
    'A private partial work',
  );
  await f.git('add', 'partial.txt');
  await f.git('commit', '-m', 'A checkpoint');
  ...
  await laneGit('add', 'plan.md');
  await laneGit('commit', '-m', 'A recovered in new verification lane');
  run = await restarted.tick(run.id, 'restored-session');
  assert.equal(run.tasks?.['1']?.state, 'accepted');
  ...
  await writeFile(
    planPathOf(run),
    (await readFile(planPathOf(run), 'utf8')).replace('- [ ] C', '- [x] C'),
  );
  await laneGit('add', 'plan.md');
  await laneGit('commit', '-m', 'C after A recovered');
  run = await restarted.tick(run.id, 'restored-session');
  assert.equal(run.tasks?.['3']?.state, 'accepted');
  assert.equal(await readFile(f.planPath, 'utf8'), oldPlan);
  f.worker.output = 'NO_FINDINGS';
  for (let step = 0; step < 15 && run.status === 'running'; step++)
    run = await restarted.tick(run.id, 'restored-session');
  assert.equal(
    run.status,
    'completed',
    run.error ?? 'pipeline did not complete',
  );
  assert.equal(run.worktreeCwd, f.root);
  assert.equal(run.branch, 'feature');
  assert.equal(
    await f.git('show', `${source.checkpointRef}:partial.txt`),
    'A private partial work',
  );
  assert.equal(
    await readFile(join(f.root, 'private-artifact'), 'utf8'),
    'private bytes stay untouched',
  );
});
...
test('independent external prerequisites wait together and retry automatically', async (t) => {
  ...
  for (
    let attempt = 0;
    attempt < 12 && run.tasks?.['1']?.state !== 'waiting_external';
    attempt++
  )
    run = await f.controller.tick(run.id, 'session');
  for (
    let attempt = 0;
    attempt < 12 && run.tasks?.['2']?.state !== 'waiting_external';
    attempt++
  )
    run = await f.controller.tick(run.id, 'session');
  assert.equal(run.status, 'running');
  ...
  const nextAttemptAt = Math.min(
    firstTask.nextAttemptAt,
    secondTask.nextAttemptAt,
  );
  ...
  assert.ok(
    run.tasks?.['1']?.nextAttemptAt !== undefined &&
      run.tasks['1'].nextAttemptAt > Date.now(),
  );
  assert.ok(
    run.tasks?.['2']?.nextAttemptAt !== undefined &&
      run.tasks['2'].nextAttemptAt > Date.now(),
  );
  vi.advanceTimersByTime(nextAttemptAt - Date.now());
  for (
    let attempt = 0;
    attempt < 12 && f.worker.launches.length === launches;
    attempt++
  )
    run = await f.controller.tick(run.id, 'session');
...
test('fair scheduler has no fifty-task or attempt cap', async () => {
  ...
  for (let id = 2; id <= 60; id++)
    states[String(id)] = { ...required(states[String(id)]), state: 'accepted' };
...
test('controller accepts fifty-one distinct committed tasks without a global iteration cap', async (t) => {
  ...
  for (let id = 1; id <= count; id++) {
    ...
    await writeFile(f.planPath, content);
    await f.git('add', 'plan.md');
    await f.git('commit', '-m', `task ${id}`);
    run = await f.controller.tick(run.id, 'session');
    assert.equal(
      run.tasks?.[String(id)]?.state,
      'accepted',
      run.wakeReason ?? 'task was not accepted',
    );
    commits.add(required(run.acceptedHead));
  }
...
test('two malformed reviews remain required with scheduled recovery', async (t) => {
  ...
  f.worker.state = 'complete';
  f.worker.proof = true;
  f.worker.output = 'Looks good overall';
...
test('restart after candidate commit verifies it without another implementation worker', async (t) => {
  ...
  await writeFile(f.planPath, '### Task 1: A\n- [x] A\n');
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'candidate before crash');
...
test('untracked source cannot be smuggled into an accepted candidate', async (t) => {
  ...
  await writeFile(f.planPath, '### Task 1: A\n- [x] A\n');
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'incomplete candidate');
  await writeFile(
    join(f.root, 'forgotten-source.ts'),
...
test('start freezes mandatory checks before a worker can remove their declarations', async (t) => {
  ...
  await f.git('add', 'package.json');
  await f.git('commit', '-m', 'trusted project checks');
  ...
  assert.deepEqual((await registry.get(started.id))?.config.requiredChecks, [
    ['npm', 'run', 'test'],
  ]);
});
...
test('required review cannot be waived by explicit skip', async (t) => {
  const f = await fixture(t);
  const run = await f.registry.update({
    ...f.run,
    stage: 'comprehensive_review',
    status: 'paused',
  });
  await assert.rejects(
    f.controller.skip(run.id, 'session', 'skip it'),
    /Required comprehensive_review/,
  );
...
for (const finalizeEnabled of [true, false]) {
  test(`final verification cannot be skipped with finalizeEnabled=${finalizeEnabled}`, async (t) => {
    ...
    await assert.rejects(
      f.controller.skip(run.id, 'session', 'Skip optional finalizer', 1),
      /Required finalize/,
    );
    ...
    for (let tick = 0; tick < 30 && run.status !== 'completed'; tick++)
      run = await f.controller.tick(run.id, 'session');
...
test('restart of an older pending finalize skip retains mandatory verification', async (t) => {
  ...
  let run = await f.registry.update({
    ...
    pendingStageSkip: {
      stage: 'finalize',
      reason: 'Older optional finalizer waiver',
      requestedAt: 1,
      requestedBy: 'session',
    },
    tasks: {
...
for (const phase of ['cancelled', 'failed'] as const) {
  test(`lost optional review reply recovers ${phase} identity but waits for retirement across restart`, async (t) => {
    ...
    const backend = {
      ...fusion,
      start: async () => {
        assert.fail('Skip cannot start another review');
      },
      cancel: async () => {
        cancels++;
        return ok({
      ...
      status: async () =>
        ok({
          ...
          ...(retired
            ? {
                processTerminalProof: {
                  ...observedWorkerProof(runId, digest),
    ...
    for (
      let tick = 0;
      tick < 30 && run.status !== 'completed_with_findings';
      tick++
    )
      run = await restarted.tick(run.id, 'session');
  ...
  const worker = new (class extends Worker {
    override async operation(
      ...
      if (!owner)
        return {
          success: false,
          error: { message: 'operation requires owner' },
        };
      owners.push(owner);
...
for (const service of ['bridge', 'fusion'] as const) {
  for (const replyKind of [
    ...
    test(`optional ${service} skip reconciles ${replyKind} lookup after cancellation reply loss`, async (t) => {
      ...
      const data =
        replyKind === 'fenced'
          ...
          : {
              ...
              terminal: true,
              operationId:
                replyKind === 'wrong operation'
                  ? 'another-operation'
                  : operationId,
              requestDigest:
                replyKind === 'wrong digest' ? 'another-digest' : digest,
              effectiveExecutionLifetime:
                replyKind === 'wrong lifetime'
                  ? { mode: 'bounded', timeoutMs: 1000 }
                  : params.executionLifetime,
            };
      const worker = Object.assign(f.worker, {
        cancelOperation: async () => lostReply,
        operation: async (_id: string, owner?: BridgeOperationOwner) => {
      ...
      const backend = {
        ...fusion,
        cancel: async () => lostReply,
        status: async () => ok(data),
      };
      const controller = new PlanExecController(
        f.registry,
        worker,
      ...
      let run = await f.registry.update({
        ...
        activeOperation: {
          operationId,
          service,
          kind: service === 'fusion' ? 'fusion' : 'review',
          params,
          requestDigest: digest,
        },
        config: { ...f.run.config, reviewRequired: false },
      });
      run = await controller.skip(
        run.id,
        'session',
...
for (const stage of ['comprehensive_review', 'stats'] as const) {
  test(`explicit skip of paused optional ${stage} continues automatically`, async (t) => {
    ...
    run = await f.controller.skip(
      run.id,
      'session',
      'Optional stage explicitly waived',
      1,
    );
...
test('paused optional review skip survives restart and waits for child retirement', async (t) => {
  ...
  let run = await f.registry.update({
    ...
    config: { ...f.run.config, reviewRequired: false },
    reviewFindings: [
      { id: 'unresolved', severity: 'MAJOR', summary: 'Still blocking' },
    ],
  });
  run = await f.controller.tick(run.id, 'session');
...
test('a newer pause wins over optional skip completion', async (t) => {
  ...
  f.registry.updateIfCurrent = async (
    ...
    if (
      !paused &&
      candidate.skippedStages.length > 0 &&
      !candidate.pendingStageSkip
    ) {
      paused = true;
      const current = await f.registry.get(run.id);
      assert.ok(current);
      await update(
        { ...current, status: 'paused', userStopped: true, stopGeneration: 2 },
        current.updatedAt,
      );
    }
...
test('paused runs observe a tracked child without accepting it or dispatching a replacement', async (t) => {
  ...
  await writeFile(f.planPath, '### Task 1: A\n- [x] A\n');
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'paused worker completed');
...
test('accepted candidate completes required review and archives only the verified commit', async (t) => {
  ...
  await writeFile(f.planPath, '### Task 1: A\n- [x] A\n');
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'verified candidate');
  f.worker.state = 'complete';
  f.worker.proof = true;
  run = await f.controller.tick(run.id, 'session');
  ...
  for (let attempt = 0; attempt < 8 && run.status === 'running'; attempt++)
    run = await f.controller.tick(run.id, 'session');
  assert.equal(
    run.status,
    'completed',
  ...
  assert.match(
    await readFile(join(f.root, 'completed', 'plan.md'), 'utf8'),
    /\[x\]/,
  );
});
...
test('cumulative native usage is counted once across repeated status polls', async (t) => {
  ...
  run = await f.controller.tick(run.id, 'session');
  run = await f.controller.tick(run.id, 'session');
  assert.deepEqual(run.tasks?.['1']?.usage, {
    inputTokens: 10,
    outputTokens: 20,
    cost: 0.5,
  });
...
test('malformed optional token counts and overflowing totals do not block worker observation', async (t) => {
  ...
  run = await f.registry.update({
    ...run,
    usage: { ...run.usage, inputTokens: Number.MAX_SAFE_INTEGER },
    tasks: {
      ...run.tasks,
      '1': {
        ...required(run.tasks?.['1']),
...
test('explicit resume adopts an approved legacy plan hash at the current revision', async (t) => {
  ...
  const paused = await f.registry.update({
    ...f.run,
...
test('stop arriving while a resume claims ownership supersedes that resume', async (t) => {
  ...
  f.registry.claim = async (candidate, sessionId) => {
    const stopped = await f.registry.update({
      ...candidate,
      status: 'paused',
      userStopped: true,
      stopGeneration: 2,
...
test('default reporting summarizes durable usage without launching a statistics worker', async (t) => {
  const f = await fixture(t);
  const stats = await f.registry.update({
    ...f.run,
    stage: 'stats',
  ...
  assert.match(
    result.statsReport?.summary ?? '',
    /input tokens: 11; output tokens: 7; cost: 0.25/,
  );
  assert.equal(f.worker.launches.length, 0);
});
...
test('optional statistics failure needs exit proof but cannot block mandatory completion afterward', async (t) => {
  ...
  let run = await f.registry.update({
    ...f.run,
    stage: 'stats',
    config: { ...f.run.config, statsEnabled: true },
  });
  run = await f.controller.tick(run.id, 'session');
  const operationId = run.activeOperation?.operationId;
  f.worker.state = 'failed';
...
test('repeated blocking findings across commit churn schedule a changed fix without waiving review', async (t) => {
  ...
  run = await f.controller.tick(run.id, 'session');
  assert.equal(f.worker.launches.length, launches);
  vi.advanceTimersByTime(required(run.nextAttemptAt) - Date.now());
  const restarted = new PlanExecController(
    f.registry,
    f.worker,
    fusion,
  ...
  assert.match(
    String(f.worker.launches.at(-1)?.params.task),
    /Change the diagnosis before editing again/,
  );
  await writeFile(join(f.root, 'churn.txt'), 'serialized write verified\n');
  await f.git('add', 'churn.txt');
  await f.git('commit', '-m', 'fix shared state race');
  run = await f.controller.tick(run.id, 'session');
  assert.equal(run.activeOperation?.kind, 'review');
  f.worker.output = 'NO_FINDINGS';
  run = await f.controller.tick(run.id, 'session');
...
test('normal progress files do not dirty candidates but neighboring user files still do', async (t) => {
  ...
  await writeFile(f.planPath, '### Task 1: A\n- [x] A\n');
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'candidate with progress');
  f.worker.state = 'complete';
  f.worker.proof = true;
  await writeFile(
    join(f.root, '.ralphex', 'user-secret.txt'),
  ...
  run = await f.registry.update({
    ...
    tasks: {
      ...run.tasks,
      '1': { ...required(run.tasks?.['1']), nextAttemptAt: 0 },
    },
  });
  run = await f.controller.tick(run.id, 'session');
  run = await f.controller.tick(run.id, 'session');
  assert.equal(
    run.tasks?.['1']?.state,
    'accepted',
    run.error ?? run.wakeReason ?? 'not accepted',
  );
});
...
test('a failed resumed candidate is cleared before a replacement attempt commits', async (t) => {
  ...
  await writeFile(f.planPath, '### Task 1: A\n- [x] A\n');
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'candidate one');
  ...
  await f.git('commit', '-m', 'candidate two');
...
test('a candidate cannot reopen the committed checkbox of an already accepted task', async (t) => {
  ...
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'A accepted');
  f.worker.state = 'complete';
  f.worker.proof = true;
  run = await f.controller.tick(run.id, 'session');
  run = await f.controller.tick(run.id, 'session');
  ...
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'B wrongly reopens A');
  run = await f.controller.tick(run.id, 'session');
...
test('archive resumes an already staged rename without misreading its second pathname', async (t) => {
  ...
  assert.equal(
    run.status,
    'completed',
    run.error ?? 'archive did not complete',
  );
  assert.equal(await f.git('status', '--porcelain'), '');
});
...
test('accepted internal-lane work fast-forwards the original output branch without erasing dirty files', async (t) => {
  ...
  let run = await f.registry.update({
    ...
    worktreeCwd: lane,
    branch: 'internal-output',
    planPath: join(lane, 'plan.md'),
    acceptedHead: candidate,
    progressPath: join(lane, progressRelativePath),
    outputTarget: {
      cwd: f.root,
      branch: 'feature',
  ...
  assert.equal(
    await readFile(join(f.root, 'user-work.txt'), 'utf8'),
    'do not overwrite',
  );
  ...
  assert.equal(
    await readFile(join(f.root, 'accepted.txt'), 'utf8'),
    'accepted independent work',
  );
  assert.match(
    await readFile(join(f.root, progressRelativePath), 'utf8'),
    /original progress preserved/,
  );
  run = await f.controller.tick(run.id, 'session');
  assert.equal(run.stage, 'stats');
...
test('authoritative absent review intent replays the exact durable request after restart', async (t) => {
  ...
  let run = await f.registry.update({
    ...f.run,
    stage: 'comprehensive_review',
    config: { ...f.run.config, reviewBackend: 'fusion' },
  });
  const controller = new PlanExecController(
    f.registry,
    f.worker,
  ...
  assert.deepEqual(backend.calls[0]?.context, {
    cwd: f.root,
    reviewedCommit: await f.git('rev-parse', 'HEAD'),
  });
});
...
test('durable never-started review cancellation fences delayed replay without wrapper exit proof', async (t) => {
  ...
  assert.ok(operationId, 'operation id missing');
...
test('output fast-forward recovery reconciles a lost reply and preserves a concurrent stop', async (t) => {
  ...
  const crashCommand: typeof runCommands = async (cwd, commands, options) => {
    ...
    if (!lost) {
      lost = true;
      throw new LocalOperationUnknownError('lost fast-forward reply');
    }
  };
...
test('final checks get a fresh persisted identity after proven nonzero exit', async (t) => {
  ...
  const execute: typeof runCommands = async (_cwd, _commands, options) => {
    ...
    if (identities.length === 1)
      throw new LocalOperationFailedError('exit 1 with owned tree retired');
  };
  run = await new PlanExecController(
    f.registry,
    f.worker,
...
test('unknown final-check result reconciles the same identity across restart', async (t) => {
  ...
  const execute: typeof runCommands = async (_cwd, _commands, options) => {
    ...
    if (identities.length === 1)
      throw new LocalOperationUnknownError(
        'lost result; original verifier may still own descendants',
      );
  };
  run = await new PlanExecController(
    f.registry,
...
test('bootstrap changes identity only after proven failure and does not consume task attempts', async (t) => {
  ...
  let run = await f.registry.update({
    ...
    config: {
      ...f.run.config,
      bootstrapCommands: [['node', '-e', 'process.exit(0)']],
    },
  });
  const identities: string[] = [];
  const execute: typeof runCommands = async (_cwd, _commands, options) => {
    identities.push(options.operationId);
    if (identities.length === 1)
      throw new LocalOperationUnknownError('bootstrap result unavailable');
    if (identities.length < 4)
      throw new LocalOperationFailedError(
        'bootstrap exited nonzero with tree retired',
      );
  };
...
test('late successful final checks cannot overwrite a newer stop generation', async (t) => {
  ...
  const execute: typeof runCommands = async (_cwd, _commands, options) => {
    identities.push(options.operationId);
    authorizations.push(options.authorization?.stopGeneration ?? 0);
    if (identities.length === 1) {
      const current = await f.registry.get(run.id);
      assert.ok(current);
      await f.registry.update({
...
test('candidate checks retain unknown ownership then use a new verifier for a proven failed attempt', async (t) => {
  ...
  const execute: typeof runCommands = async (_cwd, _commands, options) => {
    ...
    if (identities.length === 1)
      throw new LocalOperationUnknownError('candidate check result lost');
    if (identities.length === 2)
      throw new LocalOperationFailedError(
        'same check later proves nonzero exit',
      );
  };
  const controller = new PlanExecController(
    f.registry,
  ...
  await writeFile(f.planPath, '### Task 1: A\n- [x] A\n');
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'candidate awaiting checks');
  const candidate = await f.git('rev-parse', 'HEAD');
  f.worker.state = 'complete';
  f.worker.proof = true;
  run = await controller.tick(run.id, 'session');
...
test('confirmed bounded expiry grows later budgets while a lost reply preserves the exact request', async (t) => {
  ...
  for (let expiry = 1; expiry <= 2; expiry++) {
    ...
    assert.deepEqual(run.activeOperation?.expectedLifetime, {
      mode: 'bounded',
      timeoutMs: 1_000 * 2 ** expiry,
    });
    assert.deepEqual(run.config.executionLifetime, {
      mode: 'bounded',
      timeoutMs: 1_000,
    });
    assert.match(
      String(f.worker.launches.at(-1)?.params.task),
      /Resume the preserved checkpoint/,
    );
    if (expiry === 1) {
...
test('bounded compatibility stops growing at the supported timer maximum without ending recovery', async (t) => {
  ...
  assert.match(
    String(f.worker.launches[0]?.params.task),
    /narrow the session's scope and checkpoint earlier/,
  );
  assert.equal(run.status, 'running');
});
...
test('structured tool diagnosis preserves the session and schedules probes without unsupported repair', async (t) => {
  ...
  f.worker.activity = {
    ...
    toolCallId: 'tool-1',
    runnerPid: 4242,
    currentToolStartedAt: eventAt,
    lastActivityAt: eventAt,
    lastToolActivityAt: eventAt,
    lastToolFailure: {
  ...
  run = await f.controller.tick(run.id, 'session');
  const diagnosed = required(run.activeOperation?.diagnostics);
  ...
  assert.equal(
    diagnosed.lastToolFailure?.message,
    'ENOENT: build executable missing',
  );
  assert.ok(diagnosed.nextProbeAt >= diagnosed.observedAt);
  run = await f.controller.tick(run.id, 'session');
  assert.equal(
    run.activeOperation?.diagnostics?.failureKey,
    diagnosed.failureKey,
  );
  f.worker.failStatus = true;
  run = await f.controller.tick(run.id, 'session');
  assert.equal(
...
test('doctor preserves the same launch identity when an empty lookup races a late spawn', async (t) => {
  ...
  run = await new PlanExecController(
    ...
    command,
    f.localExecutor,
  ).resume(run.id, 'restarted-session', true);
  assert.equal(run.activeOperation?.operationId, operationId);
  assert.equal(run.activeOperation?.externalRunId, operationId);
  assert.equal(f.worker.launches.length, 1);
});
...
test('doctor retains a retired worker candidate and consumes its result without repeating implementation', async (t) => {
  ...
  await writeFile(f.planPath, '### Task 1: A\n- [x] A\n');
  await f.git('add', 'plan.md');
  await f.git('commit', '-m', 'candidate before doctor recovery');
  const candidate = await f.git('rev-parse', 'HEAD');
  run = await f.registry.update({
    ...run,
    tasks: {
  ...
  f.worker.state = 'complete';
  f.worker.proof = true;
  const proof = processTerminalProof(
    observedWorkerProof(
      required(operation.externalRunId),
      required(operation.requestDigest),
    ),
    required(operation.externalRunId),
    {
  ...
  assert.ok(proof);
  await execReconcile(f.registry, async () => ({
...
test('confirmed tool guidance replays one durable action after reply loss and never marks the tool repaired', async (t) => {
  ...
  worker.activity = {
    ...
    lastToolFailure: {
      ...
      toolName: 'bash',
      observedAt: Date.now(),
      message: 'build command exited 1',
    },
  };
  let controller = new PlanExecController(
    f.registry,
...
test('pause fences a child whose spawn reply is still pending', async (t) => {
  ...
  await entry;
  let run = required(await f.registry.get(f.run.id));
  const operationId = run.activeOperation?.operationId;
  run = await f.registry.update({
    ...run,
    status: 'paused',
...
test('initial worktree creation records ownership before any Git mutation', async (t) => {
  ...
  const execute: typeof runCommands = async (cwd, commands, options) => {
    ...
    if (options.operationId.startsWith('git:worktree:'))
      assert.equal(cwd, f.root);
    else {
      assert.equal(cwd, persisted?.lanePreparation?.cwd);
      assert.equal(
        persisted?.lanePreparation?.publication?.planHash,
        persisted?.planHash,
      );
    }
    await f.localExecutor(cwd, commands, options);
  ...
  assert.equal(
    run.lanePreparation?.state,
    'bootstrap',
    run.error ?? 'worktree creation did not settle',
  );
  assert.equal(
    new Set(invocations.filter((id) => id.startsWith('git:worktree:'))).size,
    1,
  );
  assert.equal(
    new Set(invocations.filter((id) => id.startsWith('plan:publish:'))).size,
    1,
  );
  assert.match(
    await readFile(join(run.worktreeCwd, '.git'), 'utf8'),
    /gitdir:/,
  );
});
...
test('bounded expiry without recent useful progress retries with a changed strategy but unchanged budget', async (t) => {
  ...
  assert.match(
    String(f.worker.launches.at(-1)?.params.task),
    /0 had recent verified model\/tool progress qualifying for growth/,
  );
  assert.match(
    String(f.worker.launches.at(-1)?.params.task),
    /Change the previous approach/,
  );
});
...
test('cancel waits for local command retirement before releasing the worktree fence', async (t) => {
  ...
  const running = runLocalOperation(
    f.root,
    [[process.execPath, '-e', localCommand]],
    {
      journalRoot: join(f.root, '.git', 'local-jobs'),
      runId: f.run.id,
      operationId: 'verification',
      activeDirectory: f.registry.localOperationsPath(f.run.id),
      authorization: {
        path: f.registry.authorizationPath(f.run.id),
        stopGeneration: 0,
      },
      isAuthorized: async () =>
        (await f.registry.get(f.run.id))?.status === 'running',
    },
  ).then(
    () => undefined,
    (error: unknown) => error,
  ...
  try {
    ...
    for (;;) {
      try {
        ...
        if (Date.now() > deadline)
          throw new Error('Local command was not started.');
      }
      await delay(10);
    }
    let run = required(await f.registry.get(f.run.id));
    ...
    run = await f.controller.advance(required(await f.registry.get(run.id)));
    assert.equal(run.status, 'cancelled');
