...
test('compatibility deadline display follows the persisted attempt rather than the frozen base', () => {
  const run = inFlight({
    ...
    activeOperation: {
      ...
      kind: 'implementation',
      launchStartedAt: 1,
      expectedLifetime: { mode: 'bounded', timeoutMs: 20_000 },
      effectiveLifetime: { mode: 'bounded', timeoutMs: 20_000 },
    },
  });
  assert.deepEqual(activeExecutionLifetime(run), {
    mode: 'bounded',
    timeoutMs: 20_000,
  });
  assert.equal(longRunningOperation(run, 10_001), undefined);
  assert.deepEqual(longRunningOperation(run, 20_002), {
    elapsedMs: 20_001,
    boundMs: 20_000,
  });
});
...
test('legacy unknown operation lifetime is not synthesized from a changed frozen base', () => {
  ...
  required(run.activeOperation).params = {
    executionLifetime: { mode: 'bounded', timeoutMs: 10_000 },
  };
  assert.deepEqual(activeExecutionLifetime(run), {
    mode: 'bounded',
    timeoutMs: 10_000,
