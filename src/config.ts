...
export function parseRunConfig(value: unknown): FrozenRunConfig {
  if (!isRecord(value))
    throw new Error('Plan execution config must be an object.');
  const optional = [
    'workerModel',
    'reviewerModel',
    'statsModel',
  ...
  const allowed = new Set([
    ...Object.keys(DEFAULT_FROZEN_RUN_CONFIG),
    ...optional,
  ]);
  for (const key of Object.keys(value))
    if (!allowed.has(key))
      throw new Error(`Unknown plan execution setting: ${key}.`);
  const config: Record<string, unknown> = {
    ...structuredClone(DEFAULT_FROZEN_RUN_CONFIG),
    ...value,
  };
  ...
  if (
    ...
    (lifetime.mode === 'bounded' &&
      (!positiveInteger(lifetime.timeoutMs) ||
        Number(lifetime.timeoutMs) > MAX_EXECUTION_TIMEOUT_MS)) ||
    (lifetime.mode === 'unbounded' &&
      Object.keys(lifetime).some((key) => key !== 'mode'))
  )
    throw new Error(
      'executionLifetime must explicitly select unbounded or bounded with a positive timeoutMs.',
    );
  for (const key of [
    'retryDelayMs',
    'workerMaxTurns',
  ...
  if (
    typeof config.reviewBackend !== 'string' ||
    !backends.has(config.reviewBackend) ||
    !Array.isArray(config.reviewFallback) ||
    !config.reviewFallback.every(
      (backend: unknown) =>
        typeof backend === 'string' && backends.has(backend),
    )
  )
    throw new Error(
      'Review backend and fallback must name subagent, fusion or revmux.',
    );
    ...
    if (
      ...
      !config[key].every(
        (command: unknown) =>
          ...
          command.length > 0 &&
          command.every(
            (arg: unknown) => typeof arg === 'string' && arg.length > 0,
          ),
      )
    )
