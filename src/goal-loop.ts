...
export function goalHash(goal: string): string {
  return createHash('sha256')
    .update(normalizeGoalText(goal))
    .digest('hex')
    .slice(0, GOAL_HASH_LENGTH);
}
...
function tail(value: string): string {
  return value.length <= GOAL_PROMPT_TAIL_LIMIT
    ? value
    : value.slice(-GOAL_PROMPT_TAIL_LIMIT);
}
...
export function normalizeCheckOutput(output: string): string {
  return output
    ...
    .replace(
      /\b(duration_ms|duration|start at|elapsed|passed in|took)\b(\s*[:=]\s*|\s*)(\d\S*)/giu,
      '$1$2<time>',
    )
...
export function parseGoalOutcome(output: string | undefined): GoalOutcome {
  ...
  const exact = (marker: string) =>
    lines.findIndex((line) => line.trim() === marker);
  const blockerIndex = exact(TASK_FAILED_MARKER);
  if (blockerIndex >= 0) {
    const blocker = lines
      .slice(blockerIndex + 1)
      .find((line) => /^Blocker:/iu.test(line.trim()))
      ?.replace(/^Blocker:\s*/iu, '')
      .trim();
    const next = lines
      .slice(blockerIndex + 1)
      .find((line) => /^Next step:/iu.test(line.trim()))
      ?.replace(/^Next step:\s*/iu, '')
      .trim();
    return {
      kind: GOAL_OUTCOME.BLOCKED,
      summary: lines
        .filter((_, index) => index !== blockerIndex)
        .join('\n')
        .trim(),
      reason:
        [blocker, next].filter(Boolean).join(' — ') ||
  ...
  if (doneIndex >= 0)
    return {
      ...
      summary:
        lines
          .filter((_, index) => index !== doneIndex)
          .join('\n')
          .trim() || 'Worker reported the goal achieved.',
    };
  return {
    kind: GOAL_OUTCOME.CONTINUE,
    summary: text || 'Worker ended without a summary.',
  };
}
