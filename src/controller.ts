...
export class PlanExecController {
  ...
  async start(options: StartRunOptions): Promise<PlanExecRun> {
    ...
    const targetPath =
      options.existingWorktree === undefined
        ? options.cwd
    ...
    if (options.existingWorktree !== undefined) {
      ...
      if (options.useWorktree) {
        await ensureCleanForWorktree(
          this.runCommand,
          repositoryRoot,
          plan.path,
        );
        if (await pathExists(executionWorktreeCwd))
          throw new Error(`Worktree already exists: ${executionWorktreeCwd}`);
    ...
    const baselineCwd = options.useWorktree
      ? repositoryRoot
      : executionWorktreeCwd;
    const config: FrozenRunConfig = {
      ...configured,
      requiredChecks: await requiredChecks(
        baselineCwd,
        configured.requiredChecks,
      ),
      bootstrapCommands: await bootstrapCommands(
        baselineCwd,
        configured.bootstrapCommands,
      ),
    };
    const initialHead = await gitValue(this.runCommand, baselineCwd, [
      'rev-parse',
      'HEAD',
    ]);
    const run = await this.registry.create(
      ...
      {
        exclusive: true,
        ...(options.onRunAllocated
          ? { onAllocated: options.onRunAllocated }
          : {}),
      },
    );
  ...
  async startGoal(options: {
    ...
    if (dirty.length)
      throw new Error(
        'A goal run works in place and refuses a dirty worktree; commit or stash changes first.',
      );
    const configured = await resolveRunConfig(repositoryRoot);
    const config: FrozenRunConfig = {
      ...configured,
      requiredChecks: await requiredChecks(
        options.cwd,
        options.checks?.length ? options.checks : configured.requiredChecks,
      ),
    };
    if (!config.requiredChecks.length)
      throw new Error(
        'A goal needs at least one required check to prove completion; add a test or build script, or pass --check <command>.',
      );
    const initialHead = await gitValue(this.runCommand, options.cwd, [
      'rev-parse',
      'HEAD',
    ]);
    const baseBranch = await defaultBranch(this.runCommand, repositoryRoot);
    ...
    const run = await this.registry.create(
      {
        ...
        goal: {
          ...
          hash,
          iteration: 0,
          maxTurns: config.maxTaskIterations,
          noProgress: 0,
        },
  ...
  async tick(runId: string, sessionId: string): Promise<PlanExecRun> {
    const coordinated = await this.registry.withControllerLock(
      runId,
      async () => {
        const run = await this.registry.get(runId);
        ...
        try {
          return await this.advanceUnlocked(claimed);
        } catch (error) {
          return this.fail(
            claimed,
            error instanceof Error ? error.message : String(error),
  ...
  private async resumeLocked(
    ...
    if (
      explicit &&
      isGoalRun(prepared) &&
      prepared.goal.iteration >= prepared.goal.maxTurns
    ) {
      const granted = await this.registry.updateIfCurrent(
        {
          ...
          goal: {
            ...prepared.goal,
            maxTurns:
              prepared.goal.maxTurns + prepared.config.maxTaskIterations,
          },
        },
        prepared.updatedAt,
      );
    ...
    if (reviewedPlanHash !== undefined) {
      if (!explicit)
        throw new Error('Adopting a changed plan requires explicit resume.');
      if (isGoalRun(prepared))
        throw new Error('Plan adoption does not apply to goal runs.');
      const planPath = requirePlanPath(prepared);
      const content = await readFile(planPath, 'utf8');
      const approved = parsePlan(planPath, content);
      if (approved.hash !== reviewedPlanHash)
        return this.pauseForReview(prepared, PLAN_STRUCTURE_CHANGED_ERROR);
      const acceptedHead =
        prepared.acceptedHead ??
        prepared.outputTarget?.initialHead ??
        (await gitValue(this.runCommand, prepared.worktreeCwd, [
          'rev-parse',
          'HEAD',
        ]));
      const { facts } = await this.planBaseline(
        prepared,
        prepared.worktreeCwd,
        acceptedHead,
        planPath,
      );
      const materialized = parsePlan(
        planPath,
        materializeApprovedPlan(planPath, content, facts),
      );
  ...
  async rebindBranchAndResume(
    ...
    const coordinated = await this.registry.withControllerLock(runId, () =>
      this.rebindBranchAndResumeLocked(
        runId,
        sessionId,
        expectedStopGeneration,
      ),
  ...
  private async rebindBranchAndResumeLocked(
    ...
    if (
      expectedStopGeneration !== undefined &&
      (claimed.stopGeneration ?? 0) !== expectedStopGeneration
    )
      return claimed;
    if (
  ...
  private async skipLocked(
    ...
    if (
      (claimed.config.reviewRequired && isReviewStage(claimed.stage)) ||
      claimed.stage === RUN_STAGE.FINALIZE
    )
      throw new Error(
        `Required ${claimed.stage} cannot be satisfied by skipping it.`,
      );
    if (
      claimed.status !== RUN_STATUS.FAILED &&
  ...
  async advance(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    if (current.status === RUN_STATUS.PAUSED)
      return this.observePausedOperation(current);
    if (current.status === RUN_STATUS.CANCEL_PENDING)
      return this.cancel(current);
  ...
  private async advanceUnlocked(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    if (run.status === RUN_STATUS.PAUSED) {
      ...
      return run.activeOperation || run.localOperationActive
        ? this.observePausedOperation(run)
        : run;
    }
    ...
    switch (run.stage) {
      ...
      case RUN_STAGE.IMPLEMENTATION:
        return isGoalRun(run)
          ? this.advanceGoal(run)
          : this.advanceImplementation(run);
  ...
  private async advanceImplementation(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    if (!run.initialPlan && !run.approvedPlan && run.tasks) {
      ...
      const materialized = materializeApprovedPlan(
        run.planPath,
        content,
        facts,
      );
      const captured = await this.registry.updateIfCurrent(
        {
          ...run,
          initialPlan: { hash: run.planHash, content: materialized },
          tasks: reconcileTaskFacts(
            parsePlan(run.planPath, materialized),
            run.tasks,
          ),
        },
        run.updatedAt,
    ...
    const checkoutRoot = await gitCheckoutRoot(
      this.runCommand,
      run.worktreeCwd,
    );
    ...
    for (const execution of Object.values(tasks)) {
      if (
        execution.state !== RUN_STATUS.RUNNING &&
        execution.state !== 'verifying'
      )
        continue;
      if (
        run.failedOperation?.taskId === execution.taskId &&
        run.failedOperation.processTreeExited === true
      ) {
        tasks[String(execution.taskId)] = {
          ...execution,
          state: 'retry_wait',
          nextAttemptAt: Date.now(),
          reason:
            execution.reason ??
            'Previous task operation has confirmed exit; scheduling recovery.',
        };
      } else {
        return this.fail(
          run,
          `Task ${execution.taskId} has unresolved operation ownership; no replacement writer is authorized.`,
          true,
        );
      }
    }
    if (Object.values(tasks).every((task) => task.state === 'accepted')) {
      const committed = await gitValue(this.runCommand, run.worktreeCwd, [
        'show',
        `HEAD:${gitPath(relative(checkoutRoot, run.planPath))}`,
      ]);
      assertAcceptedCheckboxes(
        { ...run, tasks },
        parsePlan(run.planPath, committed),
      );
      return this.transition(
        { ...run, tasks },
        ...
        await this.registry.updateIfCurrent(
          {
            ...run,
            tasks,
            nextAttemptAt: nextTaskWake(tasks, Date.now()),
            wakeReason: 'Waiting for task prerequisites or retry',
          },
          run.updatedAt,
        )
      ).run;
    const task = required(
      plan.tasks.find((candidate) => candidate.id === selected.taskId),
    );
    const acceptedHead =
      run.acceptedHead ??
      (await gitValue(this.runCommand, run.worktreeCwd, ['rev-parse', 'HEAD']));
    if (!run.outputTarget) {
    ...
    const selectedPlanPath = selected.laneCwd
      ? join(
          await gitCheckoutRoot(this.runCommand, selected.laneCwd),
          relative(checkoutRoot, run.planPath),
        )
      : undefined;
    const staleLane =
      selectedPlanPath &&
      (await readPlan(selectedPlanPath)).hash !== run.planHash;
    if (staleLane && !run.approvedPlan)
      return this.pauseForReview(run, PLAN_STRUCTURE_CHANGED_ERROR);
    if (
      selected.laneCwd &&
      selected.baselineCommit &&
      (selected.baselineCommit !== acceptedHead || staleLane) &&
      ...
      const nextTask = {
        ...selected,
        recoveryHistory: [
          ...(selected.recoveryHistory ?? []),
          ...(selected.recoverySource ? [selected.recoverySource] : []),
        ],
        recoverySource: {
          cwd: selected.laneCwd,
          branch: required(selected.laneBranch),
          baselineCommit: selected.baselineCommit,
          headCommit: await gitValue(this.runCommand, selected.laneCwd, [
            'rev-parse',
            'HEAD',
          ]),
      ...
      return this.registry.update({
        ...run,
        tasks: { ...tasks, [String(selected.taskId)]: nextTask },
        acceptedHead,
        lanePreparation: {
          taskId: selected.taskId,
          cwd: join(dirname(checkoutRoot), `plan-exec-${run.id}-${token}`),
          branch: `plan-exec/${run.id}/${token}`,
          baselineCommit: acceptedHead,
          state: 'create',
        },
      });
    }
    if (selected.laneCwd && selected.laneCwd !== run.worktreeCwd) {
      const targetRoot = await gitCheckoutRoot(
        this.runCommand,
        selected.laneCwd,
      );
      const laneRun = await this.registry.updateIfCurrent(
        {
          ...run,
          tasks,
          acceptedHead,
          planPath: join(targetRoot, relative(checkoutRoot, run.planPath)),
          worktreeCwd: selected.laneCwd,
          branch: required(selected.laneBranch),
          ...(run.progressPath
            ? {
                progressPath: join(
                  targetRoot,
                  relative(checkoutRoot, run.progressPath),
                ),
              }
            : {}),
        },
    ...
    const occupied = Object.values(tasks).some(
      (other) =>
        other.taskId !== task.id &&
        other.state !== 'accepted' &&
        other.laneCwd === run.worktreeCwd,
    );
    ...
    if (authorized && !selected.laneCwd) {
      ...
      const materialized = parsePlan(
        run.planPath,
        materializeApprovedPlan(run.planPath, authorized.content, facts),
      );
      needsPublication =
        JSON.stringify(plan.tasks.map((entry) => entry.unchecked)) !==
    ...
    const prepared = await this.registry.updateIfCurrent(
      {
        ...run,
        tasks: {
          ...tasks,
          [String(task.id)]: {
            ...
            baselineCommit: selected.baselineCommit ?? acceptedHead,
          },
        },
        acceptedHead,
        nextAttemptAt: 0,
        wakeReason: `Task ${task.id} ready`,
      },
      run.updatedAt,
    );
  ...
  private async advanceGoal(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    return this.launchBridge(run, {
      ...
      task: goalPrompt(run, {
        ...
        ...(goal.lastCheck?.failures
          ? { checkFailure: goal.lastCheck.failures }
          : {}),
        recentCommits: await this.recentGoalCommits(run),
      }),
    });
  }
  private async recentGoalCommits(
    run: PlanExecRun,
  ): Promise<string | undefined> {
    const base = run.outputTarget?.initialHead;
    if (!base) return undefined;
    const log = await gitValue(this.runCommand, run.worktreeCwd, [
      'log',
      '--oneline',
      '--no-decorate',
      `${base}..HEAD`,
  ...
  private async finishGoal(
    ...
    const progressed =
      goal.lastCheck === undefined ||
      head !== goal.lastCheck.head ||
      fingerprint !== goal.lastCheck.fingerprint;
    const noProgress = progressed ? 0 : goal.noProgress + 1;
    const nextGoal = {
      ...goal,
      noProgress,
      lastOutcome:
        outcome.kind === GOAL_OUTCOME.DONE && failure
    ...
    if (outcome.kind === GOAL_OUTCOME.DONE && !failure) {
      ...
      if (violation)
        return this.pauseGoalForGuard({ ...run, goal: nextGoal }, violation);
      const advanced = await this.registry.updateIfCurrent(
        clearError(
          withoutOperation({
            ...run,
            goal: nextGoal,
      ...
      return run.config.reviewEnabled || run.config.reviewRequired
        ? this.transition(
            advanced.run,
            RUN_STAGE.COMPREHENSIVE_REVIEW,
            'Goal achieved; starting required review.',
          )
        : this.transition(
            advanced.run,
            RUN_STAGE.FINALIZE,
            'Goal achieved; final verification required.',
          );
    }
    const scheduled = await this.registry.updateIfCurrent(
      withoutOperation({
        ...run,
        goal: nextGoal,
        nextAttemptAt: Date.now() + run.config.retryDelayMs,
        wakeReason: failure
          ? `Goal checks failing: ${failure.text}`
          : 'Goal turn completed; continuing.',
      }),
      run.updatedAt,
    );
  ...
  private async goalCheckFailure(
    run: PlanExecRun,
    operation: ActiveOperation,
    ...
    const commands = run.config.requiredChecks.map((command) =>
      command.join(' '),
    );
    ...
    try {
      ...
      if (
        ...
        throw error;
      const details =
        error instanceof LocalOperationFailedError ? error.details : undefined;
      ...
      const fingerprint = `fail:${details?.code ?? 'unknown'}:${createHash(
        'sha256',
      )
        .update(`${commands.join('\n')}\n${summary}`)
        .digest('hex')
        .slice(0, GOAL_FINGERPRINT_LENGTH)}`;
      return { text, fingerprint };
    }
  }

  /** A goal is not complete when its diff deletes tests or disables them. */
  private async goalCompletionGuard(
    run: PlanExecRun,
  ): Promise<string | undefined> {
    const base = run.outputTarget?.initialHead;
    if (!base) return undefined;
    const diff = await this.runCommand(
      'git',
      ['diff', '--name-status', '-z', '--no-relative', base, 'HEAD'],
      run.worktreeCwd,
    );
    if (diff.code !== 0)
      return 'Cannot verify the goal diff before completion.';
    ...
    for (let index = 0; index < fields.length; ) {
      ...
      if (kind === 'R' || kind === 'C') {
        ...
        if (kind === 'R' && isTestPath(first) && !isTestPath(second))
          deletedTests.push(first);
    ...
    const added = await this.runCommand(
      'git',
      ['diff', '-U0', '--no-relative', base, 'HEAD'],
      run.worktreeCwd,
    );
    if (added.code !== 0)
      return 'Cannot verify the goal diff before completion.';
    const disabled = added.stdout
      .split('\n')
      .filter((line) => line.startsWith('+') && !line.startsWith('+++'))
      .filter((line) =>
        /(?:\.(?:skip|only)\s*\(|\b(?:xit|fit)\s*\()/u.test(line),
      );
    if (disabled.length)
      return `Goal completion disables tests: ${disabled
        .slice(0, GOAL_DISABLED_SAMPLE_LIMIT)
        .map((line) => line.slice(1).trim())
        .join(' | ')}. Restore them, then resume.`;
    return undefined;
  }
  ...
  private async pauseGoal(
    ...
    if (paused.applied)
      await appendProgressBestEffort(paused.run, `Goal paused: ${reason}`);
    return paused.run;
  }
   ...
   */
  private async pauseGoalForGuard(
    run: PlanExecRun,
    reason: string,
  ): Promise<PlanExecRun> {
    const cleared: PlanExecRun = {
      ...
      goal:
        run.goal === undefined
          ? run.goal
          : { ...run.goal, lastOutcome: `Goal completion guard: ${reason}` },
    } as PlanExecRun;
    delete cleared.verifiedCommit;
    delete cleared.reviewedCommit;
    const paused = await this.registry.updateIfCurrent(
      withoutOperation(cleared),
      run.updatedAt,
    );
    if (paused.applied)
      await appendProgressBestEffort(
        paused.run,
        `Goal paused by the completion guard: ${reason}`,
      );
    return paused.run;
  }
  ...
  private async launchReview(run: PlanExecRun): Promise<PlanExecRun> {
    if (!run.config.reviewEnabled && !run.config.reviewRequired)
      return this.transition(
        run,
        RUN_STAGE.FINALIZE,
        'Optional review disabled by configuration.',
      );
  ...
  private async promoteAcceptedOutput(
    run: PlanExecRun,
    candidate: string,
  ): Promise<PlanExecRun | undefined> {
    const target = run.outputTarget;
    if (!target || target.cwd === run.worktreeCwd) return undefined;
    if (run.config.reviewRequired && run.reviewedCommit !== candidate)
      return this.fail(
        run,
        'Output promotion requires review of the current candidate.',
      );
    ...
    try {
      if (!run.outputPromotion?.commandStarted) {
        ...
        const current = await gitValue(this.runCommand, target.cwd, [
          'rev-parse',
          'HEAD',
        ]);
        const knownHeads = new Set([
          ...
          ...Object.values(run.tasks ?? {}).flatMap((task) =>
            [
              task.acceptedCommit,
              task.recoverySource?.checkpointCommit,
              ...(task.recoveryHistory ?? []).map(
                (source) => source.checkpointCommit,
              ),
            ].filter((commit): commit is string => Boolean(commit)),
          ),
        ]);
        if (!knownHeads.has(current))
          throw new Error(
            'Original output branch changed outside accepted task commits; refusing to overwrite it.',
          );
        await gitValue(this.runCommand, target.cwd, [
          'merge-base',
          '--is-ancestor',
          current,
          candidate,
        ...
        const progress =
          target.progressRelativePath === undefined
            ? undefined
            : gitPath(
                relative(
                  targetRoot,
                  join(target.cwd, target.progressRelativePath),
                ),
              );
        if (dirty.some((path) => path !== progress))
          throw new Error(
            'Original output worktree has preserved or user changes; waiting for a safe fast-forward without resetting them.',
          );
        const intended = await this.registry.updateIfCurrent(
          {
            ...run,
            outputPromotion: {
              ...required(run.outputPromotion),
              commandStarted: true,
            },
          },
          run.updatedAt,
      ...
      await this.executeLocalCommands(
        ...
        await this.localOptions(
          run,
          `git:promote:${candidate}:${run.outputPromotion?.attempt ?? 0}`,
          candidate,
        ),
      );
      await verifyExecutionTree(
      ...
      if (
        (await gitValue(this.runCommand, target.cwd, ['rev-parse', 'HEAD'])) !==
        candidate
      )
        throw new Error(
          'Original output branch does not match the retired promotion candidate.',
        );
      const promoted = await this.registry.updateIfCurrent(
        {
          ...run,
          worktreeCwd: target.cwd,
          branch: target.branch,
          planPath: join(target.cwd, target.planRelativePath),
          ...(target.progressRelativePath
            ? { progressPath: join(target.cwd, target.progressRelativePath) }
            : {}),
          outputPromotion: {
            ...required(run.outputPromotion),
            candidate,
      ...
      return this.fail(
        error instanceof LocalOperationFailedError
          ? {
              ...
              outputPromotion: {
                ...required(run.outputPromotion),
                attempt: (run.outputPromotion?.attempt ?? 0) + 1,
  ...
  private async prepareLane(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    if (run.status === RUN_STATUS.STARTING)
      return this.registry.update({ ...run, status: RUN_STATUS.RUNNING });
    const preparation = required(run.lanePreparation);
    try {
      const sourceRoot =
        preparation.taskId === 0
          ? run.worktreeCwd
          : await gitCheckoutRoot(this.runCommand, run.worktreeCwd);
      const workerCwd = join(
        preparation.cwd,
        relative(sourceRoot, run.worktreeCwd),
      );
      const planPath = join(
        preparation.cwd,
        relative(sourceRoot, run.planPath),
      );
      if (preparation.state === 'create') {
        ...
        const head = await gitValue(this.runCommand, preparation.cwd, [
          'rev-parse',
          'HEAD',
        ]);
        if (head !== preparation.baselineCommit)
          throw new Error(
            'Lane creation found an unexpected HEAD; refusing to reuse it.',
          );
        const pending = await this.publishLanePlan(
          run,
          preparation.cwd,
          preparation.baselineCommit,
          planPath,
        ...
        if (preparation.publication?.planHash !== run.planHash)
          throw new LocalOperationFailedError(
            'Plan publication retired under an earlier approval; a fresh lane is required.',
          );
        return (
          await this.registry.updateIfCurrent(
            { ...run, lanePreparation: { ...preparation, state: 'bootstrap' } },
            run.updatedAt,
          )
        ).run;
      }
      await this.executeLocalCommands(
        workerCwd,
        await bootstrapCommands(workerCwd, run.config.bootstrapCommands),
        await this.localOptions(
          run,
          `bootstrap:${preparation.cwd}:${preparation.nextAttemptAt ?? 0}`,
        ),
      );
      const task = run.tasks?.[String(preparation.taskId)];
      if (!task && preparation.taskId !== 0)
        throw new Error('Lane preparation lost its task.');
      const prepared = {
        ...run,
        worktreeCwd: workerCwd,
        ...
        ...(run.progressPath
          ? {
              progressPath: join(
                preparation.cwd,
                relative(sourceRoot, run.progressPath),
              ),
            }
          : {}),
        ...(task
      ...
      if (
        error instanceof LocalOperationFailedError &&
        preparation.state === 'create'
      ) {
        const token = randomUUID().slice(0, LANE_TOKEN_LENGTH);
        const cwd = join(
          dirname(preparation.cwd),
          `plan-exec-${run.id}-${token}`,
        );
        ...
        return this.fail(
          {
            ...run,
            lanePreparation: fresh,
            ...(initial
              ? {
                  worktreeCwd: cwd,
                  branch,
                  planPath: join(cwd, relative(run.worktreeCwd, run.planPath)),
                  ...(run.outputTarget
                    ? { outputTarget: { ...run.outputTarget, cwd, branch } }
                    : {}),
                }
              : {}),
          },
          ...
          ? {
              ...
              lanePreparation: {
                ...
                nextAttemptAt: Math.max(
                  (preparation.nextAttemptAt ?? 0) + 1,
                  Date.now() + run.config.retryDelayMs,
                ),
              },
            }
          : run,
  ...
  private async planBaseline(
    ...
    const tracked = await gitValue(this.runCommand, checkoutRoot, [
      'ls-tree',
      '--name-only',
      commit,
      '--',
    ...
    if (tracked) {
      ...
      if (result.code !== 0)
        throw new Error(
          result.stderr.trim() || 'Unable to read the committed baseline plan.',
        );
      committed = result.stdout;
    }
    const facts =
      commit === run.outputTarget?.initialHead && run.initialPlan
        ? run.initialPlan.content
        : committed;
    return { committed, facts };
  }

  private async publishLanePlan(
    run: PlanExecRun,
    cwd: string,
    ...
    assertPlanRun(run);
    const options = await this.localOptions(
      run,
      `plan:publish:${cwd}`,
      baselineCommit,
    );
    ...
    if (publication) {
      await this.executeLocalCommands(
        ...
        [
          [
            process.execPath,
            fileURLToPath(new URL('./plan-publisher.mjs', import.meta.url)),
            join(directory, `${publication.digest}.json`),
            publication.digest,
          ],
        ],
        options,
    ...
    if (!authorized) {
      const content = await readFile(
        run.lanePreparation?.sourcePlanPath ?? run.planPath,
        'utf8',
      );
      if (parsePlan(planPath, content).hash !== run.planHash)
        return this.pauseForReview(run, PLAN_STRUCTURE_CHANGED_ERROR);
      const { facts } = await this.planBaseline(
        run,
        cwd,
        ...
        await this.registry.updateIfCurrent(
          {
            ...
            initialPlan: { hash: run.planHash, content: materialized },
            tasks: reconcileTaskFacts(
              parsePlan(planPath, materialized),
              run.tasks,
            ),
          },
          run.updatedAt,
    ...
    if (authorized.hash !== run.planHash)
      return this.pauseForReview(run, PLAN_STRUCTURE_CHANGED_ERROR);
    const { committed, facts } = await this.planBaseline(
      run,
      cwd,
      baselineCommit,
      planPath,
    ...
    const payload = {
      cwd,
      path: planPath,
      expectedContent: committed ?? null,
      content:
        run.lanePreparation?.taskId === 0 && !run.approvedPlan
          ? authorized.content
    ...
    try {
      if ((await readFile(payloadPath, 'utf8')) !== serialized)
        throw new Error('Approved plan publication payload changed.');
    } catch (error) {
      ...
      await this.registry.updateIfCurrent(
        {
          ...run,
          lanePreparation: {
            ...required(run.lanePreparation),
            publication: { planHash: authorized.hash, digest },
          },
        },
        run.updatedAt,
      )
  ...
  private async verifyCandidate(
    ...
    const finalizationAttempt =
      run.stage === RUN_STAGE.FINALIZE && !run.activeOperation
        ? (run.stageAttempts[RUN_STAGE.FINALIZE] ?? 0)
        : 0;
    ...
    if (head !== candidate)
      throw new Error('HEAD changed while verifying the candidate.');
    const dirty = await worktreeChanges(this.runCommand, run.worktreeCwd);
    const checkoutRoot = await gitCheckoutRoot(
      this.runCommand,
      run.worktreeCwd,
    );
    const progress = run.progressPath
      ? gitPath(relative(checkoutRoot, run.progressPath))
      : undefined;
    if (dirty.some((path) => path !== progress))
      throw new Error(
        'Candidate has uncommitted source changes after verification.',
      );
  }

  private async localOptions(
    run: PlanExecRun,
    operationId: string,
    candidate?: string,
  ): Promise<LocalOperationOptions> {
    const commonDir = await gitValue(this.runCommand, run.repositoryRoot, [
      'rev-parse',
      '--git-common-dir',
    ]);
    return {
      journalRoot: join(
        resolve(run.repositoryRoot, commonDir),
        'plan-exec-local',
      ),
      runId: run.id,
      operationId,
  ...
  private async launchFusion(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    if (!supportsExecution(capabilities, lifetime))
      return this.reviewFailure(
        run,
        `${run.config.reviewBackend} runtime does not support the requested explicit execution lifetime, durable lookup and process-tree proof.`,
      );
    const iteration = (run.stageAttempts[RUN_STAGE.FUSION_REVIEW] ?? 0) + 1;
    const profile =
      run.config.reviewBackend === 'revmux'
        ? run.config.revmuxProfile
        : run.config.fusionProfile;
    const operationId = randomUUID();
    const launchStartedAt = Date.now();
    const params = {
      prompt: boundedContinuationPrompt(
        run,
      ...
      reviewedCommit: await gitValue(this.runCommand, run.worktreeCwd, [
        'rev-parse',
        'HEAD',
      ]),
      ...(profile ? { profile } : {}),
    ...
    if (reply.data.requestDigest !== intended.activeOperation?.requestDigest)
      return this.failUnknownLaunch(
        ...
        'Review backend did not attest the immutable request digest.',
      );
    if (
      !sameLifetime(
      ...
      return this.failUnknownLaunch(
        intended,
        intended.activeOperation,
        'Fusion did not attest the effective execution lifetime.',
      );
    const state = fusionState(reply.data);
    if (!state)
      return this.fail(
        intended,
        'Fusion launch outcome requires reconciliation.',
        true,
      );
  ...
  private async reviewFailure(
    ...
    const fallback = run.config.reviewFallback.find(
      (backend) => backend !== run.config.reviewBackend,
    );
    if (!fallback) return this.fail(run, reason);
    await appendProgressBestEffort(
      run,
      `Explicit review fallback policy selected ${fallback}: ${reason}`,
    );
    return this.fail(
      {
        ...
        config: {
          ...run.config,
          reviewBackend: fallback,
          reviewFallback: run.config.reviewFallback.filter(
            (backend) =>
              backend !== fallback && backend !== run.config.reviewBackend,
          ),
        },
      },
      reason,
  ...
  private async reviewClient(run: PlanExecRun): Promise<FusionLike> {
    if (
      (run.activeOperation?.params?.backend ?? run.config.reviewBackend) !==
      'revmux'
    )
      return this.fusion;
    const commonDir = await gitValue(this.runCommand, run.worktreeCwd, [
      'rev-parse',
      '--git-common-dir',
    ]);
    return new RevmuxReviewClient({
      cwd: text(run.activeOperation?.params?.cwd) ?? run.worktreeCwd,
      stateDirectory: join(
        resolve(run.worktreeCwd, commonDir),
        'plan-exec-review',
        run.id,
      ),
      reviewedCommit:
        text(run.activeOperation?.params?.reviewedCommit) ??
        (await gitValue(this.runCommand, run.worktreeCwd, [
          'rev-parse',
          'HEAD',
        ])),
      ...(run.config.revmuxExecutable
        ? { executable: run.config.revmuxExecutable }
        : {}),
    });
  }

  private async launchFinalizer(run: PlanExecRun): Promise<PlanExecRun> {
    const candidate = await gitValue(this.runCommand, run.worktreeCwd, [
      'rev-parse',
      'HEAD',
    ]);
      ...
      return this.fail(
        {
          ...
          stageAttempts: {
            ...run.stageAttempts,
            [RUN_STAGE.FINALIZE]:
              (run.stageAttempts[RUN_STAGE.FINALIZE] ?? 0) + 1,
          },
        },
    ...
    return this.transition(
      { ...run, verifiedCommit: candidate },
      RUN_STAGE.STATS,
      'Final verification passed for the reviewed candidate.',
    );
  }
  ...
  private async finishStats(
    ...
    const finished = await this.registry.updateIfCurrent(
      clearError(
        withoutOperation({
          ...run,
          stage: RUN_STAGE.ARCHIVE,
          nextAttemptAt: 0,
          recoveryAttempts: 0,
          statsReport: {
            state: error ? 'unavailable' : report ? 'reported' : 'summary',
            summary: report
              ? `${summary}\n${report.slice(0, MAX_TERMINAL_ERROR_LENGTH)}`
              : summary,
            ...(error ? { error } : {}),
          },
        }),
    ...
    if (finished.applied)
      await appendProgressBestEffort(
        finished.run,
        error
          ? `Optional statistics report unavailable: ${error}\n${summary}`
          : summary,
      );
    return finished.run;
  }
    ...
    const persisted = await this.registry.updateIfCurrent(
      {
        ...
        ...(input.taskId && run.tasks?.[String(input.taskId)]
          ? {
              tasks: {
                ...run.tasks,
                [String(input.taskId)]: {
                  ...
                  operationId,
                  attempts:
                    (run.tasks[String(input.taskId)]?.attempts ?? 0) + 1,
                },
              },
            }
          : {}),
  ...
  private async recoverActiveOperation(
    ...
    if (isFencedReviewCancellation(reply.data, operation)) {
      ...
      return run.status === RUN_STATUS.CANCEL_PENDING
        ...
        : this.fail(
            cleared,
            'Review launch was durably fenced before dispatch; scheduling a fresh operation.',
          );
    }
    if (
      reply.data.state === EXTERNAL_OPERATION_STATE.ABSENT &&
      reply.data.replaySafe === true &&
      ...
      if (run.status !== RUN_STATUS.RUNNING || run.userStopped)
        return this.failUnknownLaunch(
          run,
          operation,
          "An undispatched review remains fenced by the user's stop or pause.",
        );
      const capabilities = await client.capabilities?.();
      ...
      if (
        !lifetime ||
        !expectedLifetime ||
        !sameLifetime(lifetime, expectedLifetime) ||
        !supportsExecution(capabilities, lifetime) ||
        !params ||
        ...
        !text(params.reviewedCommit)
      )
        return this.failUnknownLaunch(
          run,
          operation,
          'Review replay lacks a supported runtime or matching immutable intent.',
        );
      ...
      const replay = await client.start(
        operation.operationId,
        required(text(params.prompt)),
        text(params.profile),
        lifetime,
        operation.requestDigest,
        {
          cwd: required(text(params.cwd)),
          reviewedCommit: required(text(params.reviewedCommit)),
        },
      );
      if (!replay.success)
        return this.failUnknownLaunch(latest, operation, replay.error.message);
      if (
        replay.data.requestDigest !== operation.requestDigest ||
        !sameLifetime(
          parseExecutionLifetime(replay.data.effectiveExecutionLifetime),
          lifetime,
        )
      )
        return this.failUnknownLaunch(
          latest,
          operation,
          'Replayed review did not attest its original digest and lifetime.',
        );
      ...
      if (!replayed)
        return this.failUnknownLaunch(
          latest,
          operation,
          'Review replay returned no durable run identity.',
        );
      return this.updateActiveOperation(latest, operation.operationId, {
        externalRunId: replayed.runId,
        recovery: OPERATION_RECOVERY.OBSERVE,
        expectedLifetime,
        effectiveLifetime: lifetime,
      });
    ...
    if (
      ...
      return this.failUnknownLaunch(
        run,
        operation,
        'Review lookup returned a different request digest.',
      );
    const expectedLifetime = operationExpectedLifetime(operation);
    if (
      !expectedLifetime ||
      ...
      return this.failUnknownLaunch(
        run,
        operation,
        'Review lookup did not attest the original execution lifetime.',
      );
    const state = fusionState(reply.data);
    if (!state)
      return this.failUnknownLaunch(
        run,
        operation,
        'Fusion lookup did not prove an adoptable operation; retaining ownership.',
      );
    return this.updateActiveOperation(run, operation.operationId, {
      externalRunId: state.runId,
      recovery: OPERATION_RECOVERY.OBSERVE,
  ...
  private async reconcileLocalCancellation(
    run: PlanExecRun,
  ): Promise<PlanExecRun> {
    if (!run.localOperationActive) return run;
    const result = await cancelActiveLocalOperations(
    ...
    if (
      ...
      current.stopGeneration !== run.stopGeneration
    )
      return current;
    ...
    if (result.pending) {
      ...
      updated.wakeReason =
        result.reason ?? 'Waiting for local command process-tree retirement.';
    } else {
      delete updated.localOperationActive;
      updated.nextAttemptAt = 0;
    }
    return (await this.registry.updateIfCurrent(updated, current.updatedAt))
      .run;
  }
  ...
  private async observePausedOperation(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    if (
      run.status !== RUN_STATUS.PAUSED ||
      run.stopGeneration !== original.stopGeneration
    )
      return run;
  ...
  private async observeActiveOperation(run: PlanExecRun): Promise<PlanExecRun> {
    if (run.activeOperation?.stopRequested) {
      ...
      if (run.activeOperation?.launchFenced)
        return this.recoverFencedOperation(run);
    }
    const operation = run.activeOperation;
  ...
  private async requestOperationCancellation(
    ...
    if (!operation.stopRequested) {
      const intended = await this.registry.updateIfCurrent(
        { ...run, activeOperation: { ...operation, stopRequested: true } },
        run.updatedAt,
      );
      if (!intended.applied) return intended.run;
      run = intended.run;
  ...
  private async recoverFencedOperation(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    const task = operation.taskId
      ? run.tasks?.[String(operation.taskId)]
      : undefined;
    const recoveryAttempts = (run.recoveryAttempts ?? 0) + 1;
    const retryAt =
      Date.now() + retryDelay(run.config.retryDelayMs, recoveryAttempts);
    const reason =
      operation.terminalError ??
      'The original launch was fenced before dispatch; retrying the preserved task.';
    return (
      await this.registry.updateIfCurrent(
        withoutOperation({
          ...run,
          nextAttemptAt: task ? 0 : retryAt,
          recoveryAttempts,
          wakeReason: reason,
  ...
  private async fenceRejectedLaunch(
    run: PlanExecRun,
    operation: ActiveOperation,
    data: Record<string, unknown>,
  ): Promise<PlanExecRun> {
    const rejected = await this.updateActiveOperation(
      run,
      operation.operationId,
      {
        terminalError:
          `Provider rejected the launch before dispatch: ${text(data.text) ?? 'no child was started'}`.slice(
            0,
    ...
    if (fenced.status !== RUN_STATUS.RUNNING || fenced.userStopped)
      return fenced;
    return fenced.activeOperation?.launchFenced
      ? this.recoverFencedOperation(fenced)
      : fenced;
  }
  ...
  private async observeBridge(
    ...
    if (!hasProcessExit(status.data, operation))
      return this.recordObservationFailure(
        observed,
        operation,
        'Terminal wrapper state has no confirmed process-tree exit; retaining ownership.',
      );
    observed = await this.recordBudgetExhaustion(
      observed,
      operation,
      status.data,
    );
      ...
      if (!isSuccessfulOperationState(state))
        return this.finishStats(
          ...
          operationFailureMessage(
            `Optional statistics operation ended as ${state}.`,
            terminalError,
          ),
        );
      try {
  ...
  private async observeFusion(
    ...
    if (!state?.terminal) return observed;
    if (state.runId !== operation.externalRunId)
      return this.recordObservationFailure(
        observed,
        operation,
        'Fusion status did not match the tracked run identity.',
      );
    if (!hasProcessExit(status.data, operation))
      return this.recordObservationFailure(
        observed,
        operation,
        'Fusion termination has no confirmed process-tree exit; retaining ownership.',
      );
    observed = await this.recordBudgetExhaustion(
      observed,
      operation,
    ...
    if (final.runId !== operation.externalRunId)
      return this.fail(
        current,
        'Fusion result did not match the tracked run identity.',
        true,
      );
    if (
      ...
      return this.reviewFailure(current, fusionTerminalError(final));
  ...
  private async recordObservation(
    ...
    const diagnosis = diagnoseOperation(nativeActivity, nativeStatus, {
      now: Date.now(),
      nextProbeAt: Date.now() + CONTROLLER_POLL_INTERVAL_MS,
      treeExited: nativeStatus
        ? hasProcessExit(nativeStatus, operation)
        : false,
    ...
    observedOperation.diagnostics =
      diagnosis.action === 'repair_tool'
        ...
        : diagnosis;
    const prerequisite = parseExternalPrerequisite(
      nativeStatus?.externalPrerequisite,
      'provider',
    );
    ...
    if (Object.keys(accumulated).length)
      observedOperation.reportedUsage = accumulated;
        ...
        ? {
            tasks: {
              ...
              [String(operation.taskId)]: {
                ...task,
                ...(native.lastActivityAt
                  ? { lastVerifiedActivityAt: native.lastActivityAt }
                  : {}),
  ...
  private async recordBudgetExhaustion(
    run: PlanExecRun,
    operation: ActiveOperation,
    ...
    if (
      status.terminationReason !== 'execution_lifetime_expired' ||
      lifetime?.mode !== 'bounded' ||
      run.activeOperation?.budgetExpiryRecorded
    )
      return run;
    const key = budgetKey(run, operation.kind, operation.taskId);
    const expiries = (run.budgetExhaustions?.[key] ?? 0) + 1;
    const growth = usefulProgressAtRetirement(
      ...
      await this.registry.updateIfCurrent(
        {
          ...
          activeOperation: {
            ...
            budgetExpiryRecorded: true,
            budgetGrowthGranted: growth,
          },
  ...
  private async recordObservationFailure(
    ...
    const nextProbeAt =
      Date.now() + retryDelay(run.config.retryDelayMs, failures);
    const failedRun = {
      ...run,
      ...
      activeOperation: {
        ...
        diagnostics: diagnoseOperation(undefined, undefined, {
          now: Date.now(),
          nextProbeAt,
          treeExited: false,
  ...
  private async settledFailedWorkflow(
    ...
    const restored = await this.registry.updateIfCurrent(
      clearError({
        ...
        status: RUN_STATUS.RUNNING,
        activeOperation: {
          ...required(resetOperationFailures(operation)),
          recovery: OPERATION_RECOVERY.OBSERVE,
        },
      }),
      run.updatedAt,
    ...
    if (!operation || operation.processTreeExited === true) {
      if (operation) {
        ...
        const task = operation.taskId
          ...
          : undefined;
        if (
          operation.kind === OPERATION_KIND.IMPLEMENTATION &&
          task &&
          task.state !== 'accepted'
        ) {
          const retry = {
            ...
            nextAttemptAt: Date.now(),
            reason: error,
            laneCwd: task.laneCwd ?? run.worktreeCwd,
            laneBranch: task.laneBranch ?? run.branch,
          };
          ...
          next.taskAttempts = {
            ...run.taskAttempts,
            [String(task.taskId)]:
              (run.taskAttempts[String(task.taskId)] ?? 0) + 1,
          };
        }
      }
      next = withoutOperation(next);
    }
    const paused = await this.registry.updateIfCurrent(next, run.updatedAt);
  ...
  private async finishImplementation(
    ...
    const waitingDependency = execution.dependsOn.some(
      (id) => tasks[String(id)]?.state !== 'accepted',
    );
    let reason =
      blocker ??
      terminalError ??
      `Worker ended as ${state} with incomplete task ${taskId}.`;
    if (waitingDependency)
      reason = `Task ${taskId} is waiting for its approved dependencies; preserving the worker result for recovery.`;
    if (prerequisite)
    ...
    if (
      ...
      try {
        ...
        if (candidate === execution.baselineCommit)
          throw new Error(
            'Task candidate did not advance its baseline commit.',
          );
        const checkoutRoot = await gitCheckoutRoot(
          this.runCommand,
          run.worktreeCwd,
        );
        const committedPlanText = await gitValue(
          this.runCommand,
          run.worktreeCwd,
          [
            'show',
            `${candidate}:${gitPath(relative(checkoutRoot, run.planPath))}`,
          ],
        );
        ...
        for (const source of [
          ...(execution.recoveryHistory ?? []),
          ...(execution.recoverySource ? [execution.recoverySource] : []),
        ]) {
          const checkpointCommit = await gitValue(
            this.runCommand,
            run.worktreeCwd,
            ['rev-parse', '--verify', `${source.checkpointRef}^{commit}`],
          );
          await gitValue(this.runCommand, run.worktreeCwd, [
            'merge-base',
            '--is-ancestor',
        ...
        if (
          committedPlan.hash !== run.planHash ||
          committedPlan.tasks.find((entry) => entry.id === taskId)?.unchecked
            .length !== 0
        )
          throw new Error(
            'Task checkboxes must be complete in the committed candidate plan.',
          );
        const verifying = await this.registry.updateIfCurrent(
          {
            ...run,
            tasks: {
              ...tasks,
        ...
        if (acceptedExecution.recoverySource && checkpoints.length)
          acceptedExecution.recoverySource = required(checkpoints.at(-1));
        if (acceptedExecution.recoveryHistory)
          acceptedExecution.recoveryHistory = checkpoints.slice(
            0,
            acceptedExecution.recoveryHistory.length,
          );
        delete acceptedExecution.externalPrerequisite;
        delete acceptedExecution.reason;
        delete acceptedExecution.nextAttemptAt;
        const accepted = await this.registry.updateIfCurrent(
          withoutOperation({
            ...run,
            ...
            tasks: {
              ...tasks,
              [String(taskId)]: { ...acceptedExecution, state: 'accepted' },
            },
        ...
        if (accepted.applied)
          await appendProgressBestEffort(
            accepted.run,
            `Task ${taskId} accepted at ${candidate} after verification.`,
          );
        return accepted.run;
      } catch (error) {
        if (error instanceof LocalOperationUnknownError)
          return this.fail(run, error.message, true);
    ...
    const waiting = await this.registry.updateIfCurrent(
      withoutOperation({
        ...
        taskAttempts: { ...run.taskAttempts, [String(taskId)]: attempts },
        failedOperation: { ...operation, terminalError: reason },
        wakeReason: reason,
      }),
      run.updatedAt,
    );
    if (waiting.applied)
      await appendProgressBestEffort(
        waiting.run,
        `Task ${taskId} preserved for automatic recovery: ${reason}`,
      );
    return waiting.run;
  }
  ...
  private async finishReview(
    ...
    const reviewedCommit =
      operation.reviewedCommit ?? text(operation.params?.reviewedCommit);
    if (reviewedCommit) {
      const head = await gitValue(this.runCommand, run.worktreeCwd, [
        'rev-parse',
        'HEAD',
      ...
      if (head !== reviewedCommit)
        return this.fail(
          run,
          'Reviewed candidate changed during review; a new review is required.',
        );
    }
    if (!isSuccessfulOperationState(state))
      return this.reviewFailure(
    ...
    if (
      ...
      const priorBlockers = run.reviewFindings.filter(
        (finding) => finding.severity !== 'MINOR',
      );
      const downgraded = findings.some((finding) =>
        priorBlockers.some(
          (previous) =>
            normalizedSummary(previous.summary) ===
      ...
      if (downgraded) {
        ...
        const metadata = new Set([
          ...(run.planPath
            ? [gitPath(relative(checkoutRoot, run.planPath))]
            : []),
          ...(run.progressPath
            ? [gitPath(relative(checkoutRoot, run.progressPath))]
            : []),
        ]);
        const changes = await this.runCommand(
          'git',
          [
            'diff',
            '--name-only',
        ...
        if (changes.code !== 0)
          return this.reviewFailure(
            run,
            'Unable to verify the change supporting review adjudication.',
          );
        if (
          !changes.stdout
            .split('\0')
            .some((path) => path && !metadata.has(path))
        )
          return this.reviewFailure(
            run,
            'A previously blocking finding was relabeled MINOR without a changed tree or explicit adjudication; required review remains unmet.',
          );
      }
    }
    ...
    if (!hasBlockingFindings(findings)) {
      ...
      cleared.unresolvedFindings = [
        ...run.unresolvedFindings,
        ...findings.filter((finding) => !known.has(finding.id)),
      ];
      cleared.reviewedCommit = await gitValue(
        this.runCommand,
        run.worktreeCwd,
        ['rev-parse', 'HEAD'],
      );
      await appendProgress(
        cleared,
        findings.length
          ? `${run.stage} passed with ${findings.length} reviewer-classified MINOR advisory finding(s).`
          : `${run.stage} found no issues.`,
      );
      return this.advanceUnlocked(
    ...
    const repeats =
      run.reviewRecovery?.fingerprint === fingerprint
        ? run.reviewRecovery.repeats + 1
        : 1;
    const delayed = repeats > REVIEW_DIAGNOSTIC_BURST;
    const pending = await this.registry.updateIfCurrent(
      {
        ...cleared,
        reviewRecovery: {
          fingerprint,
        ...
        nextAttemptAt: delayed
          ? Date.now() +
            retryDelay(
              run.config.retryDelayMs,
              repeats - REVIEW_DIAGNOSTIC_BURST,
            )
          : 0,
        wakeReason: delayed
          ? `The same review findings remain after ${repeats} reviews; a different diagnosis is scheduled.`
          : 'Review findings require a fix.',
      },
      run.updatedAt,
    );
      ...
      return this.fail(
        {
          ...withTerminalError(run, operation, terminalError),
          ...(run.reviewRecovery
            ? { reviewRecovery: { ...run.reviewRecovery, pendingFix: true } }
            : {}),
        },
        operationFailureMessage(
  ...
  private async archive(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    if (
      ...
      (run.progressPath !== undefined &&
        !isInsideWorktree(relative(checkoutRoot, run.progressPath)))
    )
      return this.fail(
        run,
        'Archive paths must stay inside the execution worktree.',
      );
          ...
          if (committed.code === 0) sourceTracked = true;
            ...
            throw new Error(
              committed.stderr.trim() ||
                'Could not inspect archived plan history.',
            );
        }
      }
  ...
  private async advanceArchiveOperation(
    run: PlanExecRun,
  ): Promise<PlanExecRun> {
    const operation = required(run.archiveOperation);
    if (operation.phase !== 'retired') {
      try {
        await this.executeLocalCommands(
          ...
          await this.localOptions(
            run,
            `git:archive:${operation.operationId}:${operation.attempt}`,
            run.verifiedCommit,
          ),
        );
        const current = (await this.registry.get(run.id)) ?? run;
        ...
        if (operation.phase === 'stage') {
          const pending = await this.runCommand(
            'git',
            ['status', '--porcelain', '--', ...operation.paths],
            run.worktreeCwd,
          );
          if (pending.code !== 0)
            throw new Error(
              pending.stderr || 'Cannot inspect staged archive paths.',
            );
            ...
            ? {
                ...
                commands: [
                  [
                    ...
                    '--only',
                    '-m',
                    `chore: archive ${basename(operation.destination)}`,
                    '--',
                    ...operation.paths,
                  ],
                ],
        ...
        return (
          await this.registry.updateIfCurrent(
            { ...run, archiveOperation: { ...operation, phase: 'retired' } },
            run.updatedAt,
          )
        ).run;
      } catch (error) {
        return this.fail(
          error instanceof LocalOperationFailedError
            ...
            : run,
          `Plan archival ${operation.phase}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
    const unmet = await this.completionPrerequisite(run);
    ...
    try {
      ...
      return completed.applied
        ? this.registry.release(completed.run)
        : completed.run;
    } catch (error: unknown) {
      const current = await this.registry.get(run.id);
  ...
  private async completionPrerequisite(
    run: PlanExecRun,
  ): Promise<string | undefined> {
    const head = await gitValue(this.runCommand, run.worktreeCwd, [
      'rev-parse',
    ...
    if (!run.verifiedCommit)
      return 'Required checks do not cover the current final commit.';
    if (run.config.reviewRequired && run.reviewedCommit !== run.verifiedCommit)
      return 'Required review does not cover the final verified commit.';
    if (isGoalRun(run)) {
      if (run.verifiedCommit !== head)
        return 'Current code differs from the final verified and reviewed commit.';
      const goalCheckoutRoot = await gitCheckoutRoot(
        this.runCommand,
        run.worktreeCwd,
      );
      const goalDirty = await worktreeChanges(this.runCommand, run.worktreeCwd);
      const goalProgress = run.progressPath
        ? gitPath(relative(goalCheckoutRoot, run.progressPath))
        : undefined;
      if (goalDirty.some((path) => path !== goalProgress))
    ...
    if (
      !run.tasks ||
      !Object.values(run.tasks).length ||
      Object.values(run.tasks).some((task) => task.state !== 'accepted')
    )
    ...
    if (run.verifiedCommit !== head) {
      ...
      if (
        ...
        changes.stdout
          .split('\0')
          .some((path) => path && !archivePaths.has(path))
      )
        return 'Current code differs from the final verified and reviewed commit.';
    }
    const planPath = (await pathExists(run.planPath))
      ? run.planPath
      : join(
          dirname(run.planPath),
          COMPLETED_PLANS_DIRECTORY,
    ...
    try {
      ...
      const committedPath =
        run.verifiedCommit === head ? run.planPath : planPath;
      const committed = await gitValue(this.runCommand, run.worktreeCwd, [
        'show',
        `${head}:${gitPath(relative(checkoutRoot, committedPath))}`,
      ]);
    ...
    if (
      hasBlockingFindings(run.reviewFindings) ||
      hasBlockingFindings(run.unresolvedFindings)
    )
      return 'Blocking review findings remain unresolved.';
    return undefined;
  }
    ...
    if (!operation.externalRunId) {
      if (operation.service === OPERATION_SERVICE.FUSION) {
        ...
        if (
          !state ||
          (operation.requestDigest &&
            launched.data.requestDigest !== operation.requestDigest) ||
          (launched.data.operationId !== undefined &&
            launched.data.operationId !== operation.operationId) ||
          (expectedLifetime &&
            !sameLifetime(
              parseExecutionLifetime(launched.data.effectiveExecutionLifetime),
              expectedLifetime,
            ))
        )
          return this.recordStageSkipFailure(
            run,
            operation,
            'Fusion ownership is unresolved during skip.',
          );
        const attached = await this.updateActiveOperation(
          run,
          operation.operationId,
  ...
  private async finishStageSkip(
    ...
    const ready = clearError(
      withoutOperation({
        ...
        reviewFindings: [],
        unresolvedFindings,
        skippedStages: verificationRequired
          ? run.skippedStages
          : [
              ...run.skippedStages,
              {
    ...
    const completion = await this.registry.updateIfCurrent(
      ready,
      run.updatedAt,
    );
  ...
  private async cancel(run: PlanExecRun): Promise<PlanExecRun> {
    ...
    if (
      run.activeOperation?.stopRequested &&
    ...
    if (operation.launchFenced)
      return this.registry.release(
        await this.registry.update(
          withoutOperation({ ...run, status: RUN_STATUS.CANCELLED }),
        ),
      );
    if (!operation.externalRunId)
      return this.recoverActiveOperation(run, operation);
  ...
  private async fail(
      ...
      ...withoutOperation({
        ...
        status:
          ...
          run.status === RUN_STATUS.PAUSED
            ...
            : RUN_STATUS.RUNNING,
        error,
        recoveryAttempts: (run.recoveryAttempts ?? 0) + 1,
        nextAttemptAt:
          Date.now() +
          retryDelay(run.config.retryDelayMs, (run.recoveryAttempts ?? 0) + 1),
        wakeReason: error,
        needsAttention: true,
      }),
    ...
    try {
      if (
        ...
        !(
          run.outputPromotion?.commandStarted &&
          run.outputPromotion.state === EXTERNAL_OPERATION_STATE.PENDING
        ) &&
        run.lanePreparation?.state !== 'create'
      )
        await appendProgress(
          failed,
          `Automatic recovery scheduled at ${failed.stage}: ${error}`,
        );
    } catch {
      // The registry is authoritative if the optional progress file is unavailable.
    }
...
function isFencedReviewCancellation(
  data: Record<string, unknown>,
  operation: ActiveOperation,
): boolean {
  return (
...
function supportsExecution(
  capabilities: FusionCapabilities | undefined,
...
function sameLifetime(
  actual: ExecutionLifetime | undefined,
  expected: ExecutionLifetime,
): boolean {
  return (
    actual?.mode === expected.mode &&
    (actual.mode === 'unbounded' ||
      (expected.mode === 'bounded' && actual.timeoutMs === expected.timeoutMs))
  );
}

function operationExpectedLifetime(
  operation: ActiveOperation,
...
function attemptExecutionLifetime(
  run: PlanExecRun,
  kind: ActiveOperation['kind'],
  taskId?: number,
): ExecutionLifetime {
...
function boundedContinuationPrompt(
  run: PlanExecRun,
  kind: ActiveOperation['kind'],
  prompt: string,
  lifetime: ExecutionLifetime,
...
function usefulProgressAtRetirement(
  operation: ActiveOperation,
  status: Record<string, unknown>,
  timeoutMs: number,
): boolean {
  ...
  const candidates = [
    activity?.lastModelActivityAt,
    toolAt &&
    (!activity?.lastToolFailure || activity.lastToolFailure.observedAt < toolAt)
      ? toolAt
      : undefined,
  ];
  const window = Math.min(
    BUDGET_PROGRESS_WINDOW_MS,
    Math.max(1, Math.floor(timeoutMs / BUDGET_PROGRESS_WINDOW_DIVISOR)),
  );
  return candidates.some(
    (at) =>
      at !== undefined &&
      at >= (operation.launchStartedAt ?? retiredAt) &&
      at <= retiredAt &&
      retiredAt - at <= window,
  );
}
...
function isTestPath(path: string): boolean {
  return (
    /(?:^|\/)(?:__tests__|tests?|specs?)\//iu.test(path) ||
...
function workerPrompt(
  ...
  return [
    ...
    ...(execution?.reason
      ? [
          recoveryStrategy,
          'Do not repeat an unchanged failed action without new evidence that its prerequisite recovered. If still unavailable, report the current evidence and preserve the checkpoint.',
        ]
      : []),
    ...
    ...(execution?.recoverySource
      ...
      : []),
    ...(execution?.recoveryHistory?.length
      ? [
          `Earlier preserved source checkpoints must remain recoverable and ancestral to the candidate: ${execution.recoveryHistory.map((source) => `${source.checkpointRef} (${source.cwd})`).join(', ')}.`,
        ]
      : []),
    `Already accepted tasks: ${
      Object.values(run.tasks ?? {})
        ...
        .join(', ') || 'none'
    }. Preserve their implementation and reflect their accepted checkbox completion when integrating the plan.`,
...
function retryDelay(base: number, attempts: number): number {
  return Math.min(
    ...
    base * 2 ** Math.min(Math.max(attempts - 1, 0), MAX_BACKOFF_EXPONENT),
  );
}

function parseNativeActivity(value: unknown): {
  signal?: WorkerSignal;
  ...
  const lastActivityAt =
    ...
    Number.isFinite(new Date(activity.lastActivityAt).getTime()) &&
    activity.lastActivityAt > 0
      ? activity.lastActivityAt
      : undefined;
  return {
    ...(parts.length || lastActivityAt
      ? {
          signal: {
            ...(parts.length ? { activity: parts.join(' / ') } : {}),
            ...(lastActivityAt
              ? { updated: new Date(lastActivityAt).toISOString() }
              : {}),
          },
        }
      : {}),
...
): NonNullable<ActiveOperation['reportedUsage']> {
  if (!data) return {};
  const status =
    data.statusPayload && typeof data.statusPayload === 'object'
      ? (data.statusPayload as Record<string, unknown>)
      : data;
  const tokens =
    status.totalTokens && typeof status.totalTokens === 'object'
      ? (status.totalTokens as Record<string, unknown>)
      : {};
  const cost =
    status.totalCost && typeof status.totalCost === 'object'
      ? (status.totalCost as Record<string, unknown>)
      : {};
  const input = safeTokenCount(tokens.input) ? tokens.input : cost.inputTokens;
  const output = safeTokenCount(tokens.output)
    ? tokens.output
    : cost.outputTokens;
  return {
    ...(safeTokenCount(input) ? { inputTokens: input } : {}),
    ...(safeTokenCount(output) ? { outputTokens: output } : {}),
...
function reviewFingerprint(findings: ReviewFinding[]): string {
  const normalized = [
    ...new Set(
      findings.map(
        (finding) =>
          `${finding.severity}:${normalizedSummary(finding.summary)}`,
      ),
    ),
  ].sort();
...
function reconcileTaskFacts(plan: ParsedPlan, existing: PlanExecRun['tasks']) {
  ...
  for (const task of plan.tasks) {
    const execution = required(tasks[String(task.id)]);
...
function assertAcceptedCheckboxes(run: PlanExecRun, plan: ParsedPlan): void {
  ...
  for (const task of Object.values(run.tasks ?? {})) {
    ...
    if (
      plan.tasks.find((entry) => entry.id === task.taskId)?.unchecked.length !==
      0
    )
      throw new Error(
        `Previously accepted task ${task.taskId} has reopened or missing committed checkboxes.`,
      );
  }
}
...
function statsPrompt(run: PlanExecRun): string {
  return [
    ...
    ...(run.goal
      ? [
          `Turns: ${run.goal.iteration}/${run.goal.maxTurns}`,
          `Checks: ${run.goal.lastCheck?.failures ? 'failing' : 'passing'}`,
        ]
      : []),
    `Worktree: ${run.worktreeCwd}`,
...
function taskFailureReason(output: string | undefined): string | undefined {
  ...
  return (
    report ||
    'Worker reported TASK_FAILED without a reason; inspect its output before retrying.'
  ).slice(0, MAX_TERMINAL_ERROR_LENGTH);
}

function parseExternalPrerequisite(
  value: unknown,
  source: ExternalPrerequisite['source'],
  ...
  return evidence
    ? { kind, source, evidence: evidence.slice(0, MAX_TERMINAL_ERROR_LENGTH) }
    : undefined;
}

function workerExternalPrerequisite(
  output: string | undefined,
): ExternalPrerequisite | undefined {
  if (!taskFailureReason(output)) return undefined;
  const kind = output?.match(
    /^Prerequisite:\s*(credentials|permission|missing_executable|runtime)\s*$/m,
  )?.[1];
...
export function isExternalManualBlocker(run: PlanExecRun): boolean {
  ...
  return (
    Boolean(run.blocked) ||
    recoveryEvidence(run).includes(TASK_FAILED_MARKER) ||
...
export function taskRetryRequiredMessage(run: PlanExecRun): string {
  ...
  return [
    ...
    run.blocked?.reason ??
      ...
      run.error ??
      'Inspect the worker output for the blocker.',
...
async function assertNoSymlinkPath(
  ...
  while (current !== root) {
    ...
    if (!isInsideWorktree(currentRelative))
      throw new Error(
        `Archive path escapes the execution worktree: ${target}.`,
      );
    try {
