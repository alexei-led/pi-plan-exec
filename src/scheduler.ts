...
export function selectReadyTask(tasks: Record<string, TaskExecution>): TaskExecution | undefined {
  return Object.values(tasks).filter((task) => task.state === "ready")
    .sort((a, b) => (a.lastScheduledAt ?? 0) - (b.lastScheduledAt ?? 0) || a.taskId - b.taskId)[0];
}

export function nextTaskWake(tasks: Record<string, TaskExecution>, now: number): number {
  const wakeTimes = Object.values(tasks)
    .filter((task) => task.state === "retry_wait" || task.state === "waiting_external")
    .map((task) => Math.max(task.nextAttemptAt ?? now, now));
  return wakeTimes.length ? Math.min(...wakeTimes) : now + DEPENDENCY_PROBE_DELAY_MS;
}
