...
async function main(directory, expectedDigest) {
  ...
  if (
    intent?.version !== 3 ||
    typeof intent.digest !== 'string' ||
    typeof intent.cwd !== 'string' ||
    !Array.isArray(intent.commands) ||
  ...
  for (const [index, command] of intent.commands.entries()) {
    ...
    for (;;) {
      ...
      if (grant !== undefined) {
        if (grant?.digest !== intent.digest || grant.index !== index)
          throw new Error('Invalid local command launch grant.');
        break;
      }
      await delay(POLL_MS);
    }
    if (cancelled || !(await authorized(intent, directory))) {
      cancelled = true;
      await stop();
      break;
    }
  ...
  await publish(directory, 'result.json', {
...
if (directory)
  await main(directory, process.argv[3]).catch((error) => {
