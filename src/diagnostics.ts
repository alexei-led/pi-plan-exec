...
export interface OperationDiagnostics extends OperationActivity {
  observedAt: number;
  nextProbeAt: number;
  assessment: "observing" | "tool_fault_reported" | "exit_confirmed" | "status_unavailable";
...
function label(value: unknown): string | undefined {
  ...
  return trimmed.length > 0 && trimmed.length <= MAX_LABEL_LENGTH ? trimmed : undefined;
}
...
function toolFailure(value: unknown): ToolFailureEvidence | undefined {
  ...
  if (!source || source.kind !== "tool-execution-error") return undefined;
  ...
  return toolCallId && toolName && observedAt && message
    ? { kind: "tool-execution-error", toolCallId, toolName, observedAt, message } : undefined;
}

export function parseOperationActivity(activity: unknown, status?: unknown): OperationActivity {
  ...
  for (const key of ["currentToolStartedAt", "lastActivityAt", "lastModelActivityAt", "lastToolActivityAt"] as const) {
    const value = timestamp(source[key]);
    if (value) parsed[key] = value;
  }
  const pid = source.runnerPid ?? envelope.runnerPid;
  if (typeof pid === "number" && Number.isSafeInteger(pid) && pid > 1 && pid <= MAX_PID) parsed.runnerPid = pid;
...
export function diagnoseOperation(activity: unknown, status: unknown, options: DiagnosisOptions): OperationDiagnostics {
  const parsed = options.statusUnavailable && options.prior
    ? parseOperationActivity(options.prior)
    : parseOperationActivity(activity, status);
  const failure = parsed.lastToolFailure ?? toolFailure(options.prior?.lastToolFailure);
  if (failure) parsed.lastToolFailure = failure;
  const assessment = options.treeExited ? "exit_confirmed"
    : options.statusUnavailable ? "status_unavailable"
    : failure ? "tool_fault_reported" : "observing";
  const action = options.treeExited ? "reconcile"
    : !options.statusUnavailable && failure ? "repair_tool" : "probe";
  ...
  return {
    ...
    assessment,
    action,
    ...(failure ? { failureKey: createHash("sha256").update(JSON.stringify([
      failure.kind, failure.toolCallId, failure.toolName, failure.observedAt,
    ])).digest("hex") } : {}),
    ...(error ? { error } : {}),
  };
}
